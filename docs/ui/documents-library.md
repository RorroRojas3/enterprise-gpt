...
6. **The source badge is hard-coded `Uploaded`.** No origin discriminator exists anywhere in the data model until US-1004; the stub is at one call site with a comment naming the story (§6.2, §9).
...
- **The source badge is hard-coded `<app-source-badge source="uploaded" />`.** No origin discriminator exists anywhere in the data model; US-1004 is the story that puts assistant-generated files on the wire and makes this data-driven, and the call site says so in a comment (§9).
...
- **Every row wears the `Uploaded` badge, and the assistant-created filter is hidden** — not shown-and-disabled. No origin discriminator exists anywhere in the data model; US-1004 (B7) makes the badge data-driven and US-1005 then ships the filter.
