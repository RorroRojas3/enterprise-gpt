...
| **US-1402** | The streaming answer is a polite live region that is **busy for the length of the turn**, so it is announced once rather than per delta, with a separate status region outside it carrying the running commentary (§10) |

Twelve decisions shape everything here, and each looks removable until you know what it prevents:
...
12. **The answer's live region is polite _and_ busy for the whole turn, and that pairing is the whole design.** A polite region over streaming text would read every delta; `aria-busy` is ARIA's own batching rule, so the region accumulates and exposes once. Everything else in §10 — the exclusions, the status region living outside the transcript, the spoken backstop — follows from that one attribute rather than from a throttle (§10).
...
| What a screen reader is told while a turn runs                                                                            | [`domain/stream/turn-announcement.ts`](../../enterprise-gpt-ui/src/app/domain/stream/turn-announcement.ts) — framework-free                                                                                                                                                                          |
| The region that speaks it, and the two computeds behind it                                                               | [`features/chat/chat.html`](../../enterprise-gpt-ui/src/app/features/chat/chat.html), [`features/chat/turn-store.ts`](../../enterprise-gpt-ui/src/app/features/chat/turn-store.ts)                                                                                                                   |
...
| The message footer's thumbs, and the store method that decides what a press means                                        | [`features/chat/transcript/message-feedback.ts`](../../enterprise-gpt-ui/src/app/features/chat/transcript/message-feedback.ts), [`.html`](../../enterprise-gpt-ui/src/app/features/chat/transcript/message-feedback.html), [`.scss`](../../enterprise-gpt-ui/src/app/features/chat/transcript/message-feedback.scss), [`features/chat/turn-store.ts`](../../enterprise-gpt-ui/src/app/features/chat/turn-store.ts) (`rateMessage`, `rateEntry`)                                                                                                        |
...
<div class="md-code">
  ...
  <pre class="language-ts" tabindex="0"><code class="language-ts">…</code></pre>
</div>
```
...
**The `<pre>` is a tab stop, and it is deliberately unnamed (US-1401).** `_markdown.scss` gives it `overflow: auto`, so a block wider than the column scrolls — and without `tabindex` there is no way to put focus inside it, which makes the arrow keys that would scroll it unreachable and the rest of a long line unreadable without a pointer (WCAG 2.1.1). `renderer.code` therefore emits `tabindex="0"`, `tabindex` joined `CHAT_ALLOWED_ATTR` for that one purpose, and `pre:focus-visible` takes an **inset** `--accent` ring: the `<pre>` sits flush inside the block's border, where an outline would be clipped, and the code surface is dark in both themes, which is already why the Copy control uses the accent rather than `--focus-ring`. `check-tokens.mjs` measures that pair.

Naming it looked obvious and is wrong. An `aria-label` — "ts code block" — is **invalid**: HTML-AAM maps `<pre>` to `role="generic"`, whose name ARIA 1.2 prohibits, and axe's `aria-prohibited-attr` fails it. The two ways to name it properly are both worse here — `role="region"` makes every code block a landmark, and `role="group"` buys a second entry in the sanitizer profile to announce a scroll box. Nothing is lost: `tabindex` alone is what SC 2.1.1 and axe's `scrollable-region-focusable` ask for, the block's own content is what a reader hears on arrival, and the Copy control beside it already carries "Copy ts code block". `focus-order-semantics` does not fire — it is experimental, and its `valid-scrollable-semantics` check exists for exactly this shape.

Admitting `tabindex` costs something, and the cost is stated rather than absorbed. It rests on the parser layer holding, exactly as `div` and `button` do (§3.2): a marked upgrade that routed a token kind past `renderer.html` would let model text mint a focusable element anywhere in an answer, so `markdown-security.spec.ts` drives a raw `<pre tabindex="0">` through the real pipeline as the alarm. `role` is deliberately still absent, so nothing rendered here can claim a semantic it does not have.

Three limits are recorded rather than hidden; the first two are in the [build order](../prd/enterprise-ui-rebuild-build-order.md)'s interim-behaviours table:
...
- **Every code block now costs two tab stops** — the Copy control and the `<pre>` — unconditionally, including blocks that do not overflow. That is the conventional treatment and the cheap one, but an answer with six blocks puts twelve stops between the transcript and the composer.
- **The `<pre>` announces nothing on arrival beyond its content**, per the paragraph above. A reader landing on a scroll box is told it is a scroll box by their own browser, if at all.
...
A failure leaves the source visible and appends frame `1h`'s warn-tinted notice beneath it — "This diagram could not be drawn. Its source is above." The notice's text stays `--bs-body-color` rather than `--warn`, as `TurnNoticeCard` does on the same surface. The reason it originally had to was contrast: light `--warn` measured 3.99:1 on `--warn-bg`. **US-1405 closed that upstream** — `#A96A10` became `#9C6210` in `docs/design/project/theme.css`, which measures 4.56:1 on this panel — so the constraint is gone and the treatment is kept because the board draws it that way. `check:tokens` now measures the pair in both themes either way ([Accessibility audit §6.1](accessibility-audit.md#61-light---warn-a96a10-to-9c6210)).
...
The reveal is `opacity` with four selectors, never an `@if`:
...
- `:has(.message-feedback--rated)` — US-1103's addition, and it differs from the third in kind: a copy confirmation is transient, and this is not. A rating that is only visible while the pointer happens to be over the answer is a state the reader cannot see survive a reload, which is the criterion that matters here — so a rated turn keeps its whole footer up, Copy and the token counts along with it (§9.3).
...
### 9.3 Rating an answer (US-1103)

[`MessageFeedback`](../../enterprise-gpt-ui/src/app/features/chat/transcript/message-feedback.ts) puts frame `1g`'s thumbs beside the Copy control, on a settled answer only. It holds no state of its own — a rating is an `input`, a press is an `output`, and what a press *means* is entirely `TurnStore`'s decision (below) — which is what keeps it a pure component sitting on top of the same per-message anchor US-1101 built for the token counts beside it.

**The rating lives on the transcript entry, addressed by the server's own message id.** `TranscriptEntry`'s `assistant` arm gained two fields: `messageId` (`string | null`) and `rating` (`MessageFeedbackRating | null`). `messageId` comes from two places depending on how the entry was born — the DTO's own `id` on a replayed message, and `Finished`'s `metadata.assistantMessageId` (US-1101) on a live turn that just settled — and both agree on what `null` means: **no id, no thumbs.** That covers a server old enough to send no id at all, and a completed turn whose transcript write failed, which is sent a `Finished` carrying no `metadata` object at all rather than a partial one. Withholding the controls by construction — there is nowhere to address a rating — is what closes US-1103's first criterion without a flag anyone has to remember to check.

**`replaceHistory` is the entire mechanism by which a rating survives a reload.** It rebuilds the whole replayed block from the response on every history read, so a message's `feedback.rating` ([Transcript Storage §13](../conversations/transcript-storage.md#13-the-http-surface-and-the-two-role-contracts)) lands on its entry's `rating` field the same way its `id` does, with no separate reconciliation step and nothing to keep in sync by hand.

**Three channels carry the pressed state, not one.** The glyph swaps to its `-fill` variant (`bi-hand-thumbs-up` → `bi-hand-thumbs-up-fill`, and the down pair beside it — two new entries in `icon-names.ts`, and in the sprite `npm run assets:icons` builds from it), `aria-pressed` carries the same fact for anyone reading neither, and colour reinforces both. Colour alone would fail SC 1.4.1 in dark theme, where `--muted` and `--brand` sit at a 1.06:1 luminance ratio — indistinguishable in greyscale, and to a reader with reduced colour discrimination. `check-tokens.mjs` grew two pairs for the same reason it grew the one behind US-1405's fix: `--brand` on `--surface` and on `--bs-body-bg`, the two backgrounds this selected state paints on, neither of which had ever been measured (§7.1).

**`aria-disabled`, never the native attribute — the guard is real either way, because `press()` refuses too.** A natively disabled button blurs itself the instant it is pressed, which here would strand a keyboard reader on `<body>` for the round trip, collapse the footer's own `:focus-within` reveal (§9.2) mid-request, and announce the `aria-pressed` transition to nobody — the control's only feedback channel, since it carries no visible label. The component's `press()` method checks `pending()` before emitting, so the refusal holds even though the element stays focusable and clickable.

**Pressing the thumb that is already selected is the withdrawal, and the component does not decide that — the store does.** `MessageFeedback` reports which thumb was pressed, never the resulting state; only `TurnStore.rateMessage` knows what is currently stored, so it is the one place that can tell a first rating from a withdrawal. The request either way is a `POST` carrying the state being asked for (`{ rating }`, `null` for a withdrawal) — a set, never a toggle, matching the API's own contract ([Transcript Storage §13](../conversations/transcript-storage.md#13-the-http-surface-and-the-two-role-contracts)) — so a retried or duplicated request cannot land on the opposite value.

**Ratings are cancelled on a route change, not merely forgotten.** `bindRoute` clears `pendingIds` as part of every reset it performs on a conversation change — but clearing the id without cancelling the request would leave it orphaned: the re-entry guard in `rateMessage` reads `isRowPending`, which the clear has already answered `false` for, so returning to the conversation and pressing again would race a second `POST` against the first with no guard between them. A dedicated `_rateCancel$` subject exists for exactly this, separate from `_stop$` — that one is the user's Stop, over the turn itself, and a rating in flight is not part of the turn it would be cancelling.

**On failure the store reverts to the rating captured when the button was pressed**, which a history reload landing mid-request could in principle have superseded. Left as-is deliberately: `ConversationActionsStore`'s favourite toggle has the identical shape and the identical small gap, and one bespoke reconciliation rule across two stores is worse than the shared one both already accept.

## 10. Following a streaming answer with a screen reader (US-1402)

Two of this story's four criteria look contradictory. The first asks for the streaming answer region to be `aria-live="polite"`; the third forbids announcements "so frequent as to make the live region unusable". A polite region over text that grows sixty times a second is the definition of unusable.

**They are reconciled by ARIA's own batching rule, not by a throttle.** A live region whose `aria-busy` is `true` accumulates its changes and exposes them when busy clears. So the region is polite for the whole of a streaming turn — that is criterion 1, stated literally — and busy for almost all of it, which is criteria 3 and 4 in one attribute. Nothing here counts deltas, debounces, or samples on a timer.

That single decision then dictates the rest of the section: what may sit inside the deferred region (very little), where the running commentary has to live instead (outside it), and what has to be said out loud anyway in case a screen reader does not replay what it buffered (§10.4).

### 10.1 Where the regions are, and which one is deferred

| Region                                             | Where                                                                                                    | Marked                                        | Announces                                                                                    |
| -------------------------------------------------- | -------------------------------------------------------------------------------------------------------- | --------------------------------------------- | -------------------------------------------------------------------------------------------- |
| The **streaming** answer                           | `.assistant-turn__content`, on the live turn only                                                        | `aria-live="polite"` **plus its own `aria-busy`** | The answer — once, at the moment the folded `Finished` clears busy                            |
| The **settled** answer                             | the same element on the entry that replaces it                                                           | nothing at all                                | nothing; this is what stops every answer being read twice                                    |
| The turn status                                    | [`chat.html`](../../enterprise-gpt-ui/src/app/features/chat/chat.html), **outside** `.chat__body`, last in the document | `role="status"`                               | `TurnStore.turnStatus` — the running commentary (§10.3)                                      |
| Abnormal endings, and the copy confirmation        | `transcript.html`, two separate regions                                                                  | `role="status"`                               | US-406's stopped/failed/cut-off line and US-603's "Copied" ([turn lifecycle §8](../conversations/turn-lifecycle.md#8-rendering-the-turn)) |

Five details in that table are load-bearing.

**The `aria-busy` is on the region as well as on the transcript.** `Transcript`'s host has carried one since US-406; the answer region now carries its own, because a region's own `aria-busy` is the better-honoured of the two and because it can read the folded `Finished` off the snapshot `AssistantTurn` already receives. `liveAnswer` is `streaming() && snapshot().phase !== 'Completed'`.

**It is released as an explicit `aria-busy="false"`, not by removing the attribute.** That release is the single mutation the whole pattern hangs on; leaving it to attribute removal makes the one event that matters the one event that is hardest to observe.

**`aria-atomic` is left at its default.** The region announces what changed, not the whole answer again on every change — which would restore the per-delta problem in a worse form.

**The settled turn's silence is enforced at the call site, in `transcript.html`, not inside the component.** The settled entry is a _different instance_, rendered without `[streaming]`. `AssistantTurn`'s own spec flips that input on one instance and so cannot prove the template never passes it; `transcript.spec.ts` drives a real turn to completion and asserts the replacement carries neither attribute.

**The transcript's `aria-busy` now releases on the frame, not on the body.** The host binding reads `historyPending() || (inFlight() && !answerComplete())`, where `TurnStore.answerComplete` is `snapshot.phase() === 'Completed'`. That is deliberately one step earlier than `!inFlight()`: the fold sets the phase when the `Finished` frame lands, while the settle that clears `phase` waits for the response body to close. Criterion 4 names the frame — and reading the frame is also what leaves the live turn _mounted_ at the moment its deferred region is released, which is the difference between announcing an answer and announcing nothing.

### 10.2 What is excluded, and why the exclusions are the expensive part

Activity cards and the reasoning region sit inside the answer's region, so both are marked `aria-live="off"` at their call sites in [`assistant-turn.html`](../../enterprise-gpt-ui/src/app/features/chat/transcript/assistant-turn.html).

Without that, releasing the answer flushes the accumulated state of **every card in the turn** at the reader first — each card's state label, its kind badge, its `source` subtitle and its sub-status list, all of which grow mid-turn — and then the reasoning summary, and only then the answer. Worse, the reader has already heard each of those as it happened, from the status region below. That double reading is precisely the unusable live region criterion 3 forbids, and it is reachable _only_ because criterion 1 was implemented; it does not exist in a transcript with no live region at all.

**One attribute covers the nested cards with it.** A live region covers all of its descendants, and the recursive `<app-activity-card>` declares no `aria-live` of its own, so the override on the top-level card carries down an agent tree of any depth. `transcript.spec.ts` drives a nested pair for exactly that reason — with one card the assertion would pass on the card's own attribute and prove nothing about inheritance.

Neither exclusion hides anything: both stay fully readable in browse mode, where a screen-reader user navigates the transcript by element rather than waiting to be spoken to.
...
Nothing in it is a fabricated model status: US-406's rule that the client never renders a status line the model did not send still holds, and what this reports is the client's own knowledge — a request is out, an activity is running, text has started. `Reasoning` is the apparent exception that proves it, announced only past the server's seeded delta, on the same `hasModelReasoning` test the visible region uses (US-503).

Three decisions inside the module are worth recording, because each is the kind that gets "simplified" back out.

**1. The headline follows the pre-order-last activity, not the innermost running one.** Both orderings announce every start; only this one avoids announcing the parent a second time when a nested child settles inside it — the same string twice with one thing between it and no new information in the repeat. Be precise about the cost, because the next reader will decide from it whether their case is covered: **only the pre-order-last node's transitions are announced at all.** A parent is never that node once it has a child, so its completion is silent whether or not a later sibling exists, and an earlier root's completion is silent once a later root exists. That is safe only because the two transitions worth guaranteeing survive it another way — a failure at any depth is carried by the count clause below, and the answer's arrival takes the headline itself.
...
`turnStatus` returns `'Answer ready'` from the folded `Finished` until the settle, and that overlap with the answer's own release is deliberate.

Releasing the answer's region is an `aria-busy` change **with no accompanying content mutation**. It therefore depends entirely on assistive tech having buffered what it suppressed and replaying it when busy clears. That is the specification's intent, and it is also the least reliably implemented corner of the live-region model. A reader whose AT suppressed-and-forgot would meet exactly the silence this story exists to remove; four syllables beside the answer is the cheaper of the two failures.

### 10.5 What is not verified, and the one contract this trusts

**The `aria-busy` release itself cannot be unit-tested.** jsdom has no accessibility tree and no announcement queue, so every spec here asserts the attributes and the strings — that the region is polite while streaming, that busy goes to an explicit `false` on the `Finished` frame with the live turn still mounted, that the settled instance carries nothing, that the cards are excluded, that the status region is outside both the busy container and the scroll container. **Whether a real screen reader replays the buffered answer is a manual check against NVDA, JAWS or VoiceOver, and it has not been run.** §10.4 is what makes shipping that acceptable rather than optimistic.

**`answerComplete` trusts `Finished` to be the last thing that matters.** The contract supports that — it "arrives only when the turn ran to completion" ([streaming contract §4.1](../conversations/streaming-contract.md#41-event-kinds)), which is also why that section is careful to say `Finished` is not the _terminator_: end-of-body is. What neither states and the **vendored fold does not enforce** is that no `TextDelta` follows it; the fold will happily go on appending text to a `Completed` snapshot. A server that framed a delta afterwards would leave the answer's region live and no longer busy, and every later delta would be announced on its own. The alternative is to drop those deltas — losing answer text to protect an announcement — so a wrong reading is the better failure of the two. Recorded in the build order's interim-behaviours table.

**Criterion 2 needed no work.** Toasts were already announced through US-106's two sibling regions — `role="status"` for success and info, `role="alert"` for errors and warnings — mounted once at `app.ts` and never calling `.focus()` ([Design System §8.2](design-system.md#82-live-regions)).

## 11. Testing

The suite stood at **1099 specs across 90 files** when EP-6 closed, green alongside `npm run lint` and `npm run build`. (EP-7 took it to 1153 — [Conversation Library §7](conversation-library.md#7-testing); US-1402 took it to 1689, and US-1401 to **1697**.)

| Area                             | Spec                            | Notable cases                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| -------------------------------- | ------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| The sanitizing pipeline          | `markdown-security` (27)        | The three payloads the story names — `<img src=x onerror=…>`, `<script>`, a `javascript:` href — driven through the **real** renderer and profile rather than a stub, because a spec that mocked either layer would prove nothing; raw HTML dropped with the sanitizer switched off, proving layer one alone; data attributes dropped and the `language-*` class kept; images dropped; `span` refused; the renderer overrides, including **raw HTML inside a heading**, which regressed once when the override built its own parser                                   |
| The code-block chrome            | `markdown-security` (in the 27) | The head bar naming the language and the marker landing on the `pre` as well as the `code`; an unlabelled block still offering the control; the body escaped rather than parsed; two blocks getting two _distinct_ accessible names; an info string that is not a language refused rather than escaped; — the one that guards §3.2's coupling — **raw `.md-code` chrome in the source refused**, so model text cannot aim the delegated listener; and, from US-1401, every `<pre>` carrying `tabindex="0"` and **neither an `aria-label` nor a `role`**, both halves asserted, with a raw `<pre tabindex="0">`, `<div tabindex="0">` and `<span tabindex="0">` in the source all refused, since that attribute joining the profile rests on the parser layer holding |
...
| The two renderers and the footer | `assistant-turn` (20)           | Two renderers while streaming and one once settled; closed blocks in the head and growing text in the tail; the caret on the tail, and on the head when nothing is left over; **only the tail re-parsing until a boundary is crossed**; a still-arriving code block unhighlighted and the same block highlighted at settle; **the settled render character-identical to a single full render**; and the footer's inverted gate — present without counts, absent while streaming, and **no Copy control on a turn that settled empty**; from US-1103, the thumbs withheld from an answer with no message id, offered beside Copy once one exists, withheld while still streaming regardless, the stored rating handed down for a reload, a press reported upward rather than acted on, and both thumbs marked refused while this answer's own rating is in flight                                 |
| The feedback controls            | `message-feedback` (10)          | Both thumbs named for what they mean, not the glyph; `aria-pressed` on exactly one thumb at a time; **the selected glyph swapped to its `-fill` variant, so the state is not colour alone**; a press reported for either thumb, **including a press on the one already selected** — the component reports it and leaves what it means to the store; both thumbs refused, and left natively focusable, while a rating is in flight, so the press does not throw focus away; the component marking itself rated for the footer's `:has()` reveal (§9.2) to key on; and that both controls are real, natively focusable `<button>`s with no `tabindex`, which is what the opacity-only reveal (§9.2) depends on |
| Rating an answer, wired end to end | `transcript` (4 of 51)         | The wiring the two component specs above cannot see on their own: a replayed answer offering the thumbs pre-set to the rating it came back with; a press reaching the store and **addressed to the message it belongs to**, not the conversation, asserted against the exact request body; the thumbs withheld from a cut-off turn, which was never sent a `Finished` and so claims no message id; and withheld from a completed turn whose canned `Finished` carries no `metadata` at all — the same shape a failed transcript write produces |
...
| What is announced                | `turn-announcement` (15)        | Every branch of the pure function, in Node with no `TestBed`: the create round trip; reasoning only past the seed; the headline following the newest activity **into a nested child** and reporting that child settling rather than handing back to the parent it would repeat; a settled activity giving way to the answer; an activity that starts partway through one; a failure surviving both the answer starting and a newer activity; the count net of the headline's own, at any depth, pluralised; **one string across a run of deltas**; and the recorded twelve-event turn reducing to **seven distinct strings, none repeated** |
| The regions themselves           | `assistant-turn` (2 new), `transcript` (3 new), `turn-store` (4 new), `chat` (1 new) | Polite while streaming and nothing at all once settled; busy released as an explicit `false` on the folded `Finished` **with the live turn still mounted**; the settled instance proven silent at the _call site_, which the component's own spec cannot do; the cards excluded, asserted through a **nested** pair so it is inheritance being tested; `answerComplete` true while `inFlight` still is; the `Answer ready` window and the empty string either side of it; and the status region present, empty and **outside both the busy container and the scroll container** from first render |
...
- **A screen-reader pass against real assistive technology** — US-1402 shipped the treatment in §10, but the `aria-busy` release depends on the reader replaying what it buffered, and that has not been exercised against NVDA, JAWS or VoiceOver. The `Answer ready` status line is the hedge, not the verification (§10.4, §10.5).
- **A guard against a `TextDelta` framed after `Finished`** — the fold does not enforce the contract's terminal frame, and the client deliberately keeps the text rather than dropping it (§10.5).
- **A name on the scrolling `<pre>`** — US-1401 made it a tab stop and stopped there, deliberately: `<pre>` is `role="generic"`, whose name ARIA prohibits, and giving it a real one means a role and a wider sanitizer profile for a scroll box (§3.5).
...
| A wide code block cannot be scrolled without a pointer                   | The `tabindex="0"` came off `renderer.code`, **or** `tabindex` came out of `CHAT_ALLOWED_ATTR` — the sanitizer strips it after the renderer writes it, so the two look identical in the DOM (§3.5)                                                                                                                                        |
| axe reports `aria-prohibited-attr` on a code block                       | Someone added an `aria-label` to the `<pre>`. It maps to `role="generic"`, whose name ARIA 1.2 prohibits; the Copy control beside it carries the name (§3.5)                                                                                                                                                                              |
...
| A screen reader reads the answer token by token                         | The answer region's `aria-busy` came off, or `liveAnswer` stopped reading `snapshot().phase`. Polite without busy _is_ per-delta; the pair is the design (§10)                                                                                                                                                                            |
| Every answer is announced twice                                         | `[streaming]="true"` was passed at the **settled** call site in `transcript.html`. The component is silent only because the template asks it to be (§10.1)                                                                                                                                                                                |
| Releasing the answer reads out every activity card first                | The `aria-live="off"` came off `<app-activity-card>` or `<app-reasoning-region>` in `assistant-turn.html`. Both sit inside the deferred region, and the cards' state is what gets flushed (§10.2)                                                                                                                                         |
| Nothing at all is announced between Send and the answer                 | The status region moved inside `.chat__body` or inside the transcript, where the `aria-busy` container defers it along with the answer — which is the one thing it exists not to be (§10.3)                                                                                                                                               |
| The status region says nothing on the second turn                       | `turnStatus` stopped returning `''` at rest. A region whose content never changes announces nothing, so the empty string between turns is what gives the next one something to transition from (§10.3)                                                                                                                                    |
| A parent activity's completion is never announced                       | Expected, and structural: the headline follows the **pre-order-last** activity, and a parent stops being that node the moment a child starts inside it. Failures survive it through the count clause (§10.3)                                                                                                                              |
| Deltas are announced individually after the turn finishes               | The server framed a `TextDelta` after `Finished`. The fold does not enforce the contract's terminal frame, and dropping those deltas would lose answer text to protect an announcement (§10.5)                                                                                                                                            |
