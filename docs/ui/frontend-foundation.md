...
**`incompleteSetReason` accepts `string \| Signal<string>`, resolved the same way `isAuthoritative` already was** (a plain value wrapped in a `computed`). The overload was widened for the documents library, whose ceiling notice named a different noun once the listing was narrowed to generated documents; that caller is gone (below), so every host left passes a plain string. The overload stays because the next varying host would otherwise re-derive it.

**Why it exists at all: the `/all` catalogue routes accept no sort parameter.** Sorting is therefore only ever _correct_ over a set the client holds in full there, and sorting only the page in hand is never an option — "Load more" would append a second sorted run beneath the first. Rather than leaving that condition implicit in each screen, this feature makes it a value: when `isAuthoritative` is false the sort keys are ignored, results stay in server order, and `sortUnavailableReason` carries the sentence the UI renders in place of the control. Naming the limit is the point; a silently disabled control reads as a bug.

**The documents library was its third host, and stopped being one on 2026-08-29.** Its name filter moved to `GET api/documents` when that screen adopted Load more: a client-side filter under paging searches whatever page is in hand, which is the exact failure the paragraph above rules out for sort. Two callers remain, both regime C ([Documents Library §4.3](documents-library.md#43-the-name-filter-is-served-which-is-why-the-paging-is-safe)).
...
| ---------------------------------------------- | ----------------------------------------- | --------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **A/B** — server-paged, server-ordered, server-filtered | Projects, Conversations list, Documents | Server-paged at 25 behind a Load more control on all three — `name=` always, and `sort=`/`dir=` where the endpoint accepts one, riding **every** page | Not composed on any of them. The projects grid's select is never disabled, because the order is the server's over the whole set rather than a comparator over a partial one; the conversations toolbar's select replaced US-705's stated caption ([Conversation Library §5.1](conversation-library.md#51-the-toolbar)); Documents offers no sort at all, because `GET api/documents` accepts no order ([Documents Library §4.1](documents-library.md#41-paging-and-the-drain-it-replaced)) |
...
**Regime A no longer exists as a loading strategy on any list screen.** Projects and Documents both drained to a 500-item ceiling until 2026-08-29, when the product owner reversed it: the drain rendered up to 500 rows nobody had asked for and hid everything past the ceiling behind a sentence with no way out of it. `ProjectLookupStore` still drains and must — it resolves `projectId -> name` for chips and sidebar favourites, and a paged lookup would answer `null` for any project the reader had not paged to ([Projects §9.1](projects.md#91-the-lookup-behind-it-and-why-a-failed-drain-keeps-its-pages)) — but it is not a list screen and offers no sort.
