...
4. **The grid drains rather than pages.** That was never about sorting — it is a card grid, and one that grew a page at a time would keep moving the reader's cursor. Since US-706, the order riding every drained page is the server's, not a comparator over what the client holds; `withClientQuery` was never composed once that landed (§4.1).
...
That has nothing to do with sorting: the grid is a card grid, and one that grew a page at a time would keep moving the reader's cursor. So each query walks the pages itself with `expand`, up to a stated ceiling — and since US-706, `GET api/projects` accepts `sort=` and `dir=`, which ride every page of that walk, so the 500 the ceiling admits are the correct first 500 **in the chosen order** rather than an unordered set a client would then have to sort itself.
...
- **A failure mid-drain replaces the grid with the error panel.** A half-drained set is neither complete nor knowably incomplete, and showing it — counted, or ordered as if it were the whole page-500 the notice would claim — is exactly the silently-partial list a drain exists to avoid. Retry re-runs from the first page.

The ceiling notice reads:

> Showing the first 500 of 3,412 projects, most recently updated first.

That is a truncation naming the order, not frame `4d`'s original sentence — "Sorting is unavailable past the 500-project load ceiling" — which stopped being true the moment the order became the server's: the 500 on screen are the correct first 500 in whichever order is selected, not a partial set a disabled control would have refused to reorder. The select above the grid is correspondingly **never disabled**, at the ceiling or below it (§4.3).
...
| ------------------------------ | -------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| The grid's screen              | `projects` (10)            | **Exactly one request for a deep link that already carries a term**; the term written into the URL and nothing else driving the query; back restoring the unfiltered grid; both empty states with their own way out; the error panel replacing a half-drained set; every card starred and kebabbed since US-909; and **the star's own round trip** — the label naming the direction, nothing moving until the 204, and the request body a set rather than a toggle |
...
| Missing                                                            | Owner   | Notes                                                                                                                                                    |
| ------------------------------------------------------------------ | ------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ~~**Any sort control** on the grid, and frame `4d`'s ceiling wording~~ | US-902  | Shipped, 2026-08-20 — four options over `GET api/projects`'s `sort=`/`dir=`, never disabled at the ceiling, with the notice naming the truncated order rather than frame `4d`'s original "unavailable" sentence (§4.1, §4.3) |
