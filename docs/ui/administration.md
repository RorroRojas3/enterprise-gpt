...
9. **Two controls frame `5a` draws stay absent**, each pinned by a spec: no sort control, no row-selection column (§14). The third, a "Permission: any" filter, is built (US-1206, §4.6): a `?permission=` deep link withholds the directory request until the permission catalogue answers, an id the catalogue does not list is repaired out of the URL rather than sent for the server's 400, and the empty state names whichever of the term and the permission — or both — produced it.
