...
The `/admin` area: a layout route carrying the rail its tabs hang off, and the **four** tabs behind it. A server-paged **user directory** with a create modal, a permission-editing drawer, a deactivate confirmation and a permission filter that narrows the whole directory; an unpaginated **model catalog** with a create-and-edit dialog, a set-as-default row action and a retire confirmation; an unpaginated **MCP server registry** with a create-and-edit dialog and a deactivate confirmation that has to name a cascade no other delete in this application performs; and a **usage reports** dashboard — four KPI tiles against the prior period, a usage-over-time chart, a top-ten-models bar chart, a turn-outcome donut and a paged breakdown table, none of it behind a charting library. Each of the first three splits its state across **two** stores, and that seam is the most consequential thing here. The fourth has exactly **one**, provided on the screen rather than the layout, which is §12's whole subject. The area replaces the placeholder that had stood since US-203, whose only job was to give `adminCanMatch` a real chunk to withhold.

Audience: a developer adding a **fifth** admin tab, building a chart, a numbered pager or a client-filtered table anywhere else in the app, or debugging a rejection that landed on the wrong surface. Read [Conversation Library](conversation-library.md) first for the list-screen conventions the directory copies — the URL contract, the route-scoped store, the empty states — [Authentication and Session §9](authentication-and-session.md#9-administration-withheld-rather-than-hidden-us-203) for why this chunk is withheld rather than hidden, and [Frontend Foundation](frontend-foundation.md) for the composable store features and the `core/errors` conventions everything here composes. Bare `§` references below are to sections of _this_ page.
...
9. **Two controls frame `5a` draws stay absent**, each pinned by a spec: no sort control, no row-selection column (§15). The third, a "Permission: any" filter, is built (US-1206, §4.6): a `?permission=` deep link withholds the directory request until the permission catalogue answers, an id the catalogue does not list is repaired out of the URL rather than sent for the server's 400, and the empty state names whichever of the term and the permission — or both — produced it.
...
2. **Three auth types, not frame `5h`'s, and they travel as numbers.** `McpAuthTypes` has `None = 1`, `EntraIdOnBehalfOf = 2` and `UserApiKey = 3`; "Header key" and "OAuth2" still do not exist in this system. The API registers no `JsonStringEnumConverter`, and `IsInEnum` rejects the `0` an empty select parses to — which is why a create seeds `None` rather than a blank option (§11.3).
...
9. **The Auth column is 150px, not frame `5g`'s 90px.** "Entra ID (on behalf of)" is the longest of the three values it can hold, and a narrower grid track ellipsises it permanently (§11.2).
...
11. **Headers is configuration, not a reopening of the refused "Header key" auth type — and three independent mechanisms keep it that way.** `McpServerHeaderRules` refuses `Authorization` and every other header the transport or the on-behalf-of exchange owns; `McpToolProvider` re-applies that same refusal when it builds the request rather than trusting what a row stored; and the on-behalf-of bearer is written **last**, so a stored `Authorization` can never displace it, even on a row written before the rule existed. Values round-trip in plain text to `GET api/mcps/all` and render unmasked in this dialog — nothing here is a secret store, and that is unchanged by `UserApiKey` existing as a third auth type: that arm's credential belongs to the user, not to this row, and never appears on this dialog at all (§11.3, §11.3.2). And because the PUT is a full representation, an edit that omits `headers` **clears** the stored set — which for a server registered read-only would silently restore its write tools (§11.3.2).

Ten shape the reports tab, and it took two releases, four days apart, to get there:

1. **The route shipped with US-1209, a full release before the dashboard did.** §3.2's rule — a tab is added by the story that builds its route, never ahead of it — settled a deadlock between US-1209 and US-1302 (`Depends on: US-1209`) in US-1209's favour, so `/admin/reports` existed, reachable and marked in the rail, and rendered frame `5i`'s `UnavailablePanel` until this story replaced what the panel occupied (§3.4).
2. **`ReportsStore` is provided on the screen, never the layout**, which is what makes FR-41 — "a request on every entry, never cached" — a property of DI scope rather than of code a reviewer has to trust: a component-level provider is rebuilt on every entry to the route, so nothing outlives it to serve a cached report (§12).
3. **Every model call counts in the KPI tiles and the outcome donut — a chat turn and the out-of-band naming completion alike.** That is a server decision (`GET api/reports/usage`, US-1301) the client simply renders: `totals.turns` and the donut's arcs always agree, because neither filters `Kind` the way [Usage and Favourites §7](../conversations/usage-and-favorites.md#7-reporting)'s own SQL cookbook does.
4. **No charting library.** Every mark — the ridgeline, the bars, the donut's arcs — is inline SVG or CSS, exactly as the design boards draw them, which is also what keeps the shared `admin-routes` chunk free of a dependency every other admin tab would otherwise download too (§12.3, §13.1).
5. **Two design-board corrections, both forced by contrast gates.** The area chart's line takes `--brand`, not the `--accent` frame `5j` draws — `--accent` is 2.74:1 on `--surface` and the line is the sole presentation of the daily series. The KPI delta tints only its arrow, not the figure — `--ok` at 11.5px measured 3.49:1 and failed as text (§12.3).
6. **The bar list numbers its rows.** `--brand` and `--accent` are the same colour in the dark theme, and the board's highlight-the-leader treatment collapses there without a rank a reader can follow regardless of palette (§12.3).
7. **The table's six numeric columns become proportional below 1024px.** Fixed at desktop, as frame `5j` draws them, they are wider than the content pane at tablet and would squeeze the name column to nothing — this is what retires US-1403's criterion 5, vacuous for two phases (§12.4).
8. **The footer is the shared `Paginator`, with `[liveSummary]="false"`** beside `DataTable`'s own `[summary]`, so there is exactly one live region rather than two both announcing the same count (§12.4).
9. **`?range=` always pushes a history entry; `?group=` follows the directory's own push-or-replace rule.** Turning a grouping on or off, to or from the default, is a transition and pushes; swapping one non-default grouping for another is a refinement and replaces (§12.1).
10. **`/admin/<unknown>` still redirects to `/admin/users` rather than ejecting to `/chat`, unchanged by this story.** A `**` child on the layout route, from US-1209. The consequence outlives the redirect: the layout matches by prefix, so **a new admin route must be added above that entry** or it is unreachable, and nothing reports it (§3.1).
...
| The wire shapes — `McpServerDto`, `McpServerWriteBody`, `MCP_AUTH_TYPE`     | [`domain/api/mcp.ts`](../../enterprise-gpt-ui/src/app/domain/api/mcp.ts) — the administrative shape beside the picker's `McpDto`, and the three auth types as the numbers they travel as (§11.3)                                                                                                                                 |
...
| The four parameter names, the size whitelist, the permission-id normalizer, the order statement | [`features/admin/users/admin-users-route.ts`](../../enterprise-gpt-ui/src/app/features/admin/users/admin-users-route.ts) (§4.1, §4.6)                                                                                                                                                                                                  |
...
| The reports dashboard — the screen, its store, its URL contract and its five presentational components | [`features/admin/reports/`](../../enterprise-gpt-ui/src/app/features/admin/reports/) — `admin-reports.ts`/`.html` (§12), `admin-reports-store.ts` (`ReportsStore`, §12), `admin-reports-route.ts` (the URL contract, §12.1), `usage-format.ts`, and `kpi-tile`/`outcome-donut`/`range-segmented`/`usage-area-chart`/`usage-bar-list`                        |
...
| `?permission=` | A permission id the catalogue vouches for, or the key removed for "any" (§4.6) | Pushes turning the filter on or off; **replaces** swapping one permission for another | `permissionId=` when set, omitted otherwise |

Four things about that table are decisions rather than incidentals.

**All four go out on one request.** `bindQuery` takes a single `AdminUsersQuery { name, page, pageSize, permissionId }` computation, so a deep link carrying all four issues one query rather than a default request followed by the real one. Binding them as four `rxMethod`s would fire a second query for every change to any of them. `permissionId` is the one that can make the computation answer `null` instead of a query — "not yet", not "everything" — while a `?permission=` link waits on the catalogue (§4.6).

**`size=` and `permission=`, not the API's own `take=` and `permissionId=`.** The URL is a contract with the reader rather than a proxy for the request underneath it — the same reason the conversations library spells its favourites filter `favorites=1` and not `isFavorite=true` ([Conversation Library §2.2](conversation-library.md#22-the-three-url-parameters)).

**Both numeric inputs are defensively transformed, and `?size=` is whitelisted rather than clamped.** `toPage` reads anything that is not a whole number at or above 1 as page 1 — a hand-edited `?page=0` would otherwise send `skip=-25`, and `?page=abc` a `skip=NaN`. `toPageSize` accepts only the three sizes the select offers, so the control can never display a value the URL contradicts. `toPermissionFilter` only trims and normalizes to `null`, because unlike a page size a permission id is a row an administrator created — whether it names anything is a question only the catalogue can answer, and the screen settles that one step later (§4.6).

A search resets the page (`[PAGE_PARAM]: null`), and so does a size change or a permission change: a narrowed result set rarely has as many pages as the one before it, and row 26 is page 2 at 25 per page and page 1 at 50 — an offset that means something different than it did a moment ago.

**The API still accepts more than this URL spends.** `GET api/users` has taken `sort=`/`dir=` since US-706, 2026-08-20 — but no control on this tab sends it yet, which is now the tab's own gap rather than the API's (§15). Wiring it is the same shape as `?permission=` was: fold it into the query object `bindQuery` already takes as a computation, and reset the page the same way a search does.
...
`AdminLayout` holds **no store**, and no tab may provide one on it: US-1209 requires that opening a tab instantiates that tab's store and no other. The third file is the screen itself, and which shape on this page it copies is decided by the response shape: a server-paged route takes `AdminUsers`'s URL contract (§4.1), an unpaginated collection takes `AdminModels`'s client filter (§10.2), and a route whose response is **one aggregated envelope rather than a collection** takes `AdminReports`'s — a component-scoped store with its own URL contract for the controls that reshape the request, and no `withEntities` at all (§12). `GET api/mcps/all` is unpaginated, which is why US-1208 copied the models tab and touched none of the directory's files.
...
- **Never let `UserActionsStore`, `ModelActionsStore` or `McpServerActionsStore` be reached from anything eagerly imported.** `providedIn: 'root'` is a DI scope, not a chunk assignment, and the initial graph has a fraction of a kilobyte of headroom (§13.1).
...
/admin  canMatch: [adminCanMatch]  loadChildren      ← the chunk, withheld not hidden
 └─ ''  AdminLayout                                  ← the 190px rail (pills below 768px)
      ...
      ├─ 'reports' AdminReports  providers: [ReportsStore]
      └─ '**'      → redirect to 'users'
```
...
Both criteria US-1403 asks of this area — tables to cards at 44px targets, and the rail to pills — were therefore already met when that story opened, in the narrow sense that the collapse itself fired: the rail became pills, and `DataTable` restructured into `CardRow`s. The card anatomy that mechanism produced did not hold up under a closer look, and a later pass closed six defects in it (§3.3.1). Three things about the area changed with US-1403 itself, and all three are still worth knowing:
...
#### 3.3.1 Six defects in the card anatomy, and four bugs found while fixing them

`DataTable`'s collapse mechanism worked from US-1403 onward — the breakpoint, `PillSubnav`, the restructure into `CardRow`s. What it produced was wrong in six ways no existing spec caught, because none of them measured a rendered card against a real 44px target or a real column header:

1. **The `meta` slot had no labels.** A column moved to `mobile: 'meta'` stacked its bare value — three numbers under a model name, three more under an MCP server's — with nothing saying which was which. `CardRow`'s `meta` slot is now a `<dl>`, one `<dt>`/`<dd>` pair per column keyed by `column.header`, styled from `data-table.scss` because that template is the one that declares it — no `::ng-deep` needed. A caller that does not want a label moves the column to `subtitle` instead (§10.3, §11.2).
2. **A lone kebab stretched into a full-width bar.** `mobile: 'actions'` is the card's foot, meant for a row of labelled buttons — Users is the one screen that still uses it that way, matching frame `5m` (§4.4). A single icon-only action inherited the foot's full width and read as a broken control rather than a menu. `MobileSlot` gained `trailing`, rendered beside the title inside `.card-row__main`, and the models, MCP servers, conversations and project-conversations tables all moved their icon-only kebab there — the conversations library's favourite star moved with it, since it was the same shape of problem.
3. **The 44px target rule landed on the wrapper, not the button.** `card-row.scss`'s `min-height: 44px` was written against `.card-row__actions ::ng-deep > *`, which reaches whatever a consumer projects into the foot — on the Users table that is a wrapper `<div>`, not the buttons inside it, so Edit and Deactivate stayed at their unstyled ~31px. The rule now sizes every `button` the `trailing` slot holds directly (not only the kebab — the conversations favourite star was ~21px), and `admin-users.scss` sizes its own foot buttons to 44px rather than relying on a rule two components upstream.
4. **An empty slot still cost space.** `CardRow`'s `:empty` rule never matched, because `DataTable` always projects a wrapper element into every slot — an empty slot is an empty wrapper, not nothing. The selector is now `:has(> :only-child:empty)`, `:only-child` because a slot can legitimately hold several projected nodes where only one being empty says nothing about the rest.
5. **A long value ellipsised with no way to read the rest of it.** A mono deployment name, a URL, a scope, and `PermissionBadge`'s own name all clip on the desktop table, recoverable there only by hover. A card has no hover, so all four wrap instead of ellipsising below the mobile breakpoint, and `PermissionBadge` gained `max-width: 100%` so it can shrink at all.
6. **The 768–1023px tablet range rendered every column.** `DataTable` had exactly two branches — the full table, or cards — so between the two edges a table with eight columns and no room for them simply ran wider than its pane. `TableColumn.hideOnTablet`, read by a `visibleColumns` computed the table branch alone consumes, drops the columns each table can least afford at that width: deployment, max output and tools on the models table, description, auth and scope on the MCP registry. The card branch deliberately still reads `columns()` in full — a phone is a different layout, not a narrower tablet (§10.3, §11.2).

Four more bugs surfaced while closing the six above. None is specific to this pass — all four predate it, on every card that ever hit the code path in question:

- **Any `text`-only column rendered blank on a card.** The table branch has always had a `cellText` fallback for a column with no cell template; the card branch never did.
- **A deactivated MCP server never dimmed.** The template applied `.admin-mcps__server--retired`; the stylesheet defined `.admin-mcps__name--retired`. The two never matched.
- **A model name could push past its cell at narrow desktop widths.** `.admin-models__name` had no `min-width: 0`; its MCP-table sibling, `.admin-mcps__name`, already did.
- **A long provider name could overflow `<app-status-dot>`'s cell.** The component had no `min-width: 0`/`max-width` on its host and no truncation on its label.

`expectNoHorizontalOverflow` ([Accessibility audit §4](accessibility-audit.md#4-the-harness)) gained a carve-out for the wrap-vs-ellipsis distinction defect 5 depends on, and `admin.a11y.spec.ts` gained nine viewport-fitting tests — the user directory, the model catalog and the MCP registry, each at desktop, tablet and mobile — mounted through an `openInShell` helper that narrows the routed element by the width the shell's own sidebar actually leaves at each breakpoint (260px / 60px / 0px), rather than against the bare viewport. Auditing against the bare viewport is what let defects 3 and 4 through the first time: the fitting tests below give a screen only the room production actually leaves it.

### 3.4 The reports tab: a placeholder route, then a dashboard (US-1209, US-1302)

The fourth tab exists **so that its URL does** — that was the whole point of building it in US-1209, before it had anything to render. US-1209's first criterion named `/admin/reports` among four URLs that each had to render as the active tab with the rail marking it, and a tab whose route is missing cannot be marked. §3.2's own rule — a tab is added by the story that builds its route, never ahead of it — had booked the fourth tab to US-1302, but US-1302 `Depends on: US-1209`, so each was waiting on the other; US-1209 broke the deadlock by simply being the story that built the route, with design frame `5i`, labelled "Reports — unavailable panel (current state)" and captioned "Required frame", as the authority for shipping an `<h1>` and the shared [`UnavailablePanel`](../../enterprise-gpt-ui/src/app/shared/feedback/unavailable-panel/) and nothing else in the meantime. FR-49 (P0) — every capability with no backing API renders a visible unavailable state, never placeholder figures — was what that interim state satisfied.

Until US-1302 landed, that is what `/admin/reports` was: no rows, no `ReportsStore`, and no request issued at all — a deliberate departure from a written criterion, since US-1302's own first acceptance criterion asked for a store holding zero records, and with no endpoint such a store would hold no state and load nothing, so the criterion's intent (FR-49) was satisfied more strongly by a tab that instantiated none. It was `UnavailablePanel`'s first production consumer; US-106 had built the component against frames `4i` and `5i`, and only `/ui-kit` had rendered it before then.

**US-1302 replaced the panel on 2026-08-21, once its enabler US-1301 put `GET api/reports/usage` on the wire.** Nothing about the route itself changed in the process: the URL, the rail entry, the page title and the `<h1>` are all exactly what US-1209 built. That includes `.admin__content`'s **flex-column** layout, originally written so frame `5i`'s panel could centre in whatever space the tab's `flex: 1` host was given — the dashboard needed no change to that structure, because `flex: 1` still hands the host all the room it needs, whatever now fills it. §12 is the dashboard's own section.
...
This is the one table in the area that still keeps `actions` as the card's full-width foot rather than moving to `trailing` — it holds two labelled buttons, Edit and Deactivate, which is exactly what that slot is for (§3.3.1). `admin-users.scss` sizes both to 44px directly, rather than through `card-row.scss`'s shared rule, which reaches only what a consumer projects and could not reach past the wrapper this screen hands it (§3.3.1).
...
### 4.6 The permission filter (US-1206)

The "Permission: any" select frame `5a` draws, narrowing the whole directory rather than the loaded page. Its enabler, US-1205, put `permissionId=` on `GET api/users` the day before this story wired a control to it — so frame `5a`'s interim "Filters loaded rows only." caption never rendered, and a spec asserts its permanent absence rather than its retirement (§14).

**The filter needs a catalogue before it can render options, so a `?permission=` deep link withholds the directory request.** `AdminUsers.ngOnInit` calls `UserActionsStore.ensurePermissions()` — exported from the root store for exactly this — on mount, and `bindQuery`'s computation answers `null` instead of a query while `awaitingPermissions()` is true: `permission()` is non-null and the catalogue has not yet settled. `AdminUsersStore`'s `rxMethod` filters `null` out (§5) rather than sending an unfiltered request and correcting it a moment later, which would paint the whole directory before narrowing it to the page that was actually linked to. A link carrying no `?permission=` is unaffected — `awaitingPermissions()` is false from the start, and the directory request fires exactly as it did before this story.

**"Settled" means answered, not answered with rows.** `_catalogSettled` reads `permissionsLoaded() || permissionsError() !== null`, not `permissions().length > 0` — the guard `ensurePermissions()` itself uses, where a `200 []` costing a redundant refetch is harmless. Here it would be a fixed point: an empty catalogue would leave `awaitingPermissions()` true forever, with no request in flight and nothing to retry. `permissionsLoaded` is therefore its own field on `UserActionsState`, set by `finalize` on the permissions load — true on either arm, because "has this been asked and answered" is a question a failure settles as surely as rows do.

**Three computeds decide what the request and the control show, and they disagree on purpose:**

| Computed | Answers | Reads |
| --- | --- | --- |
| `selectedPermission` | The `PermissionDto` `?permission=` names, once the catalogue vouches for it, or `null` | `permissions().find(p => p.id === id)` |
| `permissionFilter` | The `permissionId` the request actually carries | `selectedPermission()?.id ?? null` — **except** when the catalogue itself failed, where the raw id passes through unvalidated |
| `unnamedFilter` | A filter that is in force but cannot be named | Only non-null when the catalogue failed **and** `permission()` is set |

**An id the catalogue does not list reads as "any" and is repaired out of the URL** — deactivated since the link was shared, or hand-edited. This is the `?size=` invariant ("the control can never display a value the URL contradicts") enforced against a run-time set instead of a fixed one ([§2.2](#22-the-four-url-parameters)), and it is the **one** exception to the screen's own rule against navigating from an effect over a bound input: an effect watches `permission() !== null && _catalogSettled() && permissionFilter() === null` and calls `_applyQuery({ permission: null }, true)` inside `untracked`. It is safe because it terminates by construction — the navigation it performs sets the very input the condition reads to `null`, which is the condition's own negation, so the effect cannot re-fire itself. It also touches a parameter no debounced control is bound to, which is the feedback path the no-effect-navigation rule exists to prevent in the first place.

**A failed catalogue load is the one case nothing can be repaired.** `permissionFilter` sends the raw id — the server is left as the authority a filter control usually pre-empts — and the `<select>` renders `unnamedFilter()` as an option of its own labelled "Filtered permission" rather than silently falling back to "Permission: any", which would claim the request is unfiltered while it is not. Rendering it as a selectable option also gives a keyboard or screen-reader user somewhere to go: re-selecting an already-selected option fires no native `change` event, so without an option to move *off*, the reader would have no way back to the unfiltered directory but the address bar.

**The empty state gained a third arm.** `noMatchHeading`, `noMatchMessage` and `clearFiltersLabel` all branch on `_filteredByPermission` (`users.permissionId() !== null` — the **store's** copy, matching what `AdminUsersStore.hasNoMatches` itself branches on, so the heading can never describe a filter whose results have not arrived) crossed with whether a term is also in play:

| Term | Permission | Heading | Action |
| --- | --- | --- | --- |
| — | — | `No users match "term"` | Clear search |
| — | ✅ | `No users hold "Administrator"` (or `this permission`, if the catalogue cannot name it) | Clear filter |
| ✅ | ✅ | `No users match "term" and hold "Administrator"` | Clear filters |

`clearFilters()` (renamed from `clearSearch()`) clears whichever are on in **one** navigation — clearing them separately would push an intermediate, half-cleared directory onto the history stack that the reader never asked for — and moves focus to whichever control the reader was actually using: the permission select if only that filter was on, the search field otherwise. Both targets sit outside every `@if` branch in the template, so both are always connected to receive it.

**Turning the filter on or off pushes a history entry; swapping one permission for another replaces.** `onPermissionFiltered` passes `shouldReplaceHistory`'s existing rule a filter instead of a term: a closed `<select>` commits on every arrow key, so pushing unconditionally would cost a keyboard reader one history entry per permission walked past, and as many `Back` presses to escape — where a pointer, which commits once, would cost one either way.
...
Eight columns — model, provider, deployment, context, max output, tools, status, actions — mapped onto `DataTable`'s card slots so the same rows read below the breakpoint: `title`, `subtitle`, `meta` (deployment, context, max output — each labelled by its own header on a card, §3.3.1), `badges` (tools, status) and `trailing` for the kebab, which a card renders beside the model name rather than as a full-width foot (§3.3.1). Deployment, max output and tools also carry `hideOnTablet`: the eight-column table has no room for all of them between 768 and 1023px, so those three drop from the table there and stay on the card below it (§3.3.1).
...
- **The model name and the deployment name wrap on a card rather than ellipsising.** Both are read character by character when they are wrong — a deployment name especially — and a card has no hover to reopen an ellipsis with, unlike the desktop cell. `.admin-models__name` also gained `min-width: 0`, which it needs at narrow desktop widths independently of the card branch: without it the name never shrank and pushed past its own cell (§3.3.1).
...
The root store's handler calls `reload()` rather than `ensureLoaded()`: the memo is already resolved by the time an administrator can change anything, so the memoized call would hand back the stale list — and `reload()` also replaces the memo, so a later `ensureLoaded()` sees the fresh one. That handler is the whole **+0.46 kB** this story adds to the initial bundle (§13.1).
...
| Comparators    | `name`, `provider` — wired, unoffered            | `name`, `url` — wired, unoffered (§15)                                                      |
...
`GET api/mcps/{id}` exists and is **deliberately unused**. The row `mcps/all` returned already carries every field the PUT has to echo, and the single-server route answers 404 for a deactivated server — a second path to the same data with a different failure mode, which is what §15's note on the grant/revoke routes refuses on the users tab for the same reason.

### 11.2 The table, and why a deactivated server is still in it

Eight columns — server, description, URL, auth, scope, permission, status, actions — mapped onto `DataTable`'s card slots so the same rows read below the breakpoint: `title`, `subtitle`, `meta` (URL, auth, scope — each labelled by its own header on a card, §3.3.1), `badges` (permission, status) and `trailing` for the kebab (§3.3.1). Description, auth and scope also carry `hideOnTablet`, dropped from the table between 768 and 1023px for the same reason the models table drops three of its own (§3.3.1, §10.3).
...
Eight more details are load-bearing:

- **The URL, the scope and the server name wrap on a card rather than ellipsising.** All three are mono or read character by character when something is wrong — the URL most of all, since this column is the only place in the application to check one — and a card has no hover to reopen an ellipsis with, unlike the `[title]` the desktop cell carries below (§3.3.1).
...
- **The Auth column is 150px, not frame `5g`'s 90px.** The longest of the three values this column can hold is "Entra ID (on behalf of)", and at 90px it ellipsises permanently — a column that can never show one of its values in full is a column that says nothing. The board's own width was drawn against a *different* three-value enum than this system ships — "None", "Header key" and "OAuth2" rather than "None", "Entra ID (on behalf of)" and "API key (per user)" — so the coincidence of arity is exactly that, a coincidence; the case for 150px does not depend on it.
...
| `authType`    | A select over `AUTH_TYPE_OPTIONS`. **Three entries**, and they submit as numbers                                          |
...
**Three auth types, not the board's, and they travel as numbers.** `McpAuthTypes` has `None = 1`, `EntraIdOnBehalfOf = 2` and — since the per-user MCP credential work — `UserApiKey = 3`; frame `5h`'s "Header key" and "OAuth2" still do not exist in this system, the same class of board error as frame `4l`'s file extensions (§11.3.2 records why `UserApiKey` is not a late arrival of the board's own "Header key"). The API registers no `JsonStringEnumConverter`, so the enum serializes as its underlying `int`, and the validator's `IsInEnum` rejects `0` — which is why `toMcpServerFormValue(null)` seeds **`None`** rather than offering a blank option that could only ever fail. `AUTH_TYPE_OPTIONS` lives in `core/catalog/mcp-server-form.ts` rather than beside the constant in `domain/api/mcp.ts`, for `PROVIDER_OPTIONS`'s reason (§10.7): that file is reached by the composer's Tools menu, and this list has exactly one consumer on a lazy route. `UserApiKey` needs nothing else from this dialog — like `None`, it takes no scope (`requiresScope` is `true` for exactly one arm, `EntraIdOnBehalfOf`) — because the credential itself belongs to each user, not to the registration; see the [GitHub MCP Server runbook](../mcp/github-server.md) for where a user actually supplies one.
...
The select offers the marks `MCP_ICON_OPTIONS` names — "None" first, then Azure, Context7, GitHub, Microsoft and Salesforce in the array's own sorted order — and a live `app-brand-icon` preview sits beside it, decorative and unlabelled: the select's own value is what a screen reader announces, and a second reading of the same choice would announce it twice. `iconPreview` is one `computed` over the field's current value, so typing (selecting) and the preview never disagree.
...
**Values render unmasked, and that is the point rather than an oversight.** This is not the board's refused "Header key" auth type reopened, even though the Auth type select now offers a third option — that refusal was specifically about **one shared secret an administrator types into this dialog**, landing in a plaintext, administrator-readable column any other administrator who opens the row can read straight back off the screen. `UserApiKey` (§11.3) is the opposite arrangement: there is no secret on this dialog at all, because the credential belongs to the *user*, not to the registration — an administrator picks the auth type and stops there, and the token itself is supplied later by whichever user wants to use the server, stored encrypted per user, and returned by no route this or any other administrator can read. What the Headers field carries is a third, unrelated thing again: non-secret configuration a remote server reads to restrict itself, unaffected by which auth type the server uses — and masking it would misrepresent what it is.
...
The root store's handler calls `reload()` rather than `ensureLoaded()`, for §10.6's reason exactly: the memo is already resolved by the time an administrator can change anything, and `reload()` also replaces it so a later `ensureLoaded()` sees the fresh list. That handler is the whole **+0.15 kB** this story adds to the initial bundle (§13.1) — a third of US-1207's, because `withEventHandlers` and the `adminEvents` symbol were already on the graph from `ModelCatalogStore`.

## 12. The reports dashboard (US-1302)

Frames `5j` (the dashboard) and `5k` (its empty state) replace what §3.4 describes. `UnavailablePanel` is gone, because `GET api/reports/usage` (US-1301, [Usage and Favourites §7](../conversations/usage-and-favorites.md#7-reporting)) now exists. Nothing else on the route changes: the URL, the rail entry, the page title and the `<h1>` are all inherited unchanged from US-1209 (§3.1, §3.4). `ReportsStore` is the only new state anywhere in the area, and it is provided **on `AdminReports`, never on the layout** — which is what makes FR-41's "a request on every entry, never cached" a property of DI scope rather than of code a reviewer has to trust: a component-level provider is destroyed and rebuilt on every entry to the route, so a back-navigation gets a fresh instance holding no report, and its constructor-time `bindQuery` issues a request before anything can render a stale one.

Three features the sibling admin stores compose are deliberately absent from `ReportsStore`, and each is a departure worth knowing before copying its shape: **no `withEntities`**, because the response is one envelope holding five differently-shaped row sets rather than a keyed collection with an id to key on; **no `withOffsetPagination`**, because the grouped table is a numbered window over a set already in hand rather than a "Load more" running offset, so the page lives in plain state — the same call `AdminUsersStore` made for the identical reason; and **no `withEventHandlers`**, because nothing in this application mutates usage, so no event could ever invalidate a report — only the range and grouping controls do that.

### 12.1 The URL contract — `?range=` and `?group=`

`?range=7|30|90` and `?group=model|provider|user|day` are both repaired synchronously in their own input transforms, on the same terms as the catalog's `?permission=` filter: the accepted sets are compile-time constants, so an out-of-range value is corrected client-side rather than sent for the server to reject. Only `from` goes on the wire — computed as `range` days back from the browser's own clock — because the server resolves `to` and echoes both bounds back, and a client-sent `to` would let a browser with the wrong clock decide which rows count. `from`/`to` on the response are what the date chip renders, so the chip can never claim a window the figures beside it were not taken from.

The two controls push and replace on different rules, because they are not the same kind of change. **The range control always pushes** — one press is one deliberate choice, and Back should return to the previous range. **The grouping control follows the user directory's own rule**: turning a grouping on or off (to or from the default, `model`) is a transition and pushes, while swapping one non-default grouping for another is a refinement and replaces. A closed `<select>` commits on every arrow key, so replacing the swaps costs a keyboard reader one history entry for the whole walk instead of one per grouping passed through, while the push at either end is what keeps Back able to undo the change at all.

Both controls are bound to `bindQuery(() => ({ range: this.range(), grouping: this.group() }))` in `AdminReports`'s constructor — a computation rather than two separate values, so a deep link carrying both parameters issues **one** request rather than a default request followed by the real one, the same reasoning `rxMethod` is used for everywhere else in this codebase. Neither control writes the store directly: every change is a navigation, through one private `_applyQuery`, which is what keeps the URL authoritative and the back button working with no bookkeeping of its own.

### 12.2 The KPI tiles, and what a delta means

The four tiles — total tokens, estimated cost, active users, conversations — each carry a signed percentage against the equal-length prior period the server already computed (`priorTotals`). **Direction is never carried by colour alone** (SC 1.4.1): an arrow glyph and the signed percentage both state it, so `--ok`/colour is reinforcement, not the message. The board paints every rise `--ok`, cost included, and that is kept rather than re-encoded — a usage report answers "what changed", and deciding that more spend is bad would be this component inventing a product opinion the boards do not hold. An unchanged figure gets **no arrow at all**, because a glyph is a channel and must not lie by pointing somewhere; a prior period of zero gets no percentage either, and a caption says so instead — "No prior period to compare with" — because every change from nothing is mathematically infinite, and `+∞%` is not a way to report it. The estimated-cost tile is the one place a `null` figure is deliberately treated as `0` for the *delta* calculation, distinct from its own `formatCost` rendering: a period nobody could price and a period that genuinely cost nothing are the same *change* from an equally unpriced prior period, and the tile's own value already says which of the two this one was.

The delta's caption — "prior 30 days" — is derived from the **response's** window, not from `?range=`: the control flips the instant it is clicked, and until the next report lands the tiles still hold the previous period's figures. A caption naming the range just requested, before the figures for it have arrived, would describe data that is not yet on screen.

### 12.3 Three hand-drawn charts, and two contrast corrections

**No charting library.** Every mark — the area chart's ridgeline, the bar list's fills, the donut's arcs — is inline SVG or CSS, exactly as the design boards themselves draw them. A library would add its own bundle to the shared `admin-routes` chunk every admin tab downloads, to reproduce a shape that is, in the area chart's case, eleven straight line segments with no curve smoothing: a spline through daily totals would invent values between the days that were actually measured, which on a spend report is a number somebody could act on. This is also what answers the bundle note §13.1 records below.

Two corrections were forced on the boards by the contrast gates, and both are recorded in `check-tokens.mjs`'s `CONTRAST_PAIRS` (16 new rows, covering both themes) rather than left to a reviewer to notice by eye:

- **The area chart's line takes `--brand`, not the `--accent` frame `5j` draws.** `--accent` measures 2.74:1 on `--surface` in the light theme — below the 3:1 non-text minimum — and the line is the *sole* visual presentation of the daily series (SC 1.4.11), so failing it is not cosmetic. The gradient fill under the line keeps `--accent`, because a fill is decorative background rather than the graphical object carrying the data, and the ridgeline motif survives in both themes.
- **The KPI delta's tone applies only to the arrow, never to the figure.** `--ok` at 11.5px measured 3.49:1 — clears the non-text minimum an icon needs, but fails the higher bar axe applies to text. The signed percentage and the comparison caption stay in the body colour; the arrow alone carries `--ok`/`--warn`, which is also consistent with §12.2's "colour is reinforcement, not the message."

The donut is entirely decorative and the legend beside it is the data: three colours on a ring cannot be read by anyone who does not see them as three colours, and a donut has no accessible geometry `aria-label` could summarise honestly, so the ring is `aria-hidden` and every arc is also a legend row carrying its name, its count and its share through `StatusDot`. The area chart's path is `aria-hidden` on the same reasoning, with a computed `summary` sentence — span, total, peak day — standing in for it; the bar list needs no such summary, because each bar already carries its label and its value as text beside it, not only as a fill.

**The bar list numbers its rows**, a departure from the board, because `--brand` and `--accent` are the same colour in the dark theme and the board's highlight-the-leader treatment collapses there — a numbered rank survives a palette where colour cannot carry the comparison.

### 12.4 The grouped table — proportional columns below 1024px, and the shared `Paginator`

The table's first column is named after whichever grouping produced the rows on screen (`reports.renderedGrouping()`, read off the *response's* `groupBy` rather than the control, for the same staleness reason as §12.2's caption), and the other six are fixed figures — Turns, Completed, Cancelled, Failed, Tokens, Est. cost. At desktop those six sit at frame `5j`'s own fixed widths; **below the desktop edge (1024px) they become proportional instead of staying fixed**, because the six fixed tracks alone are wider than the content pane at tablet, which would squeeze the name column to nothing and clip every row. Hiding columns was the alternative and is worse — every one of them is a figure the table exists to compare. This is what **retires US-1403's criterion 5**, vacuous since that story shipped because there were no charts on this tab to stack; it is now asserted by a real browser overflow check at three widths ([Responsive layout §10](../ui/responsive-layout.md#10-limits-recorded-rather-than-hidden)), not only by the jsdom track-width assertion this table's own spec carries.

The footer is the shared `Paginator`, not the board's two bare chevrons — the same fidelity call §12.3's chart-library note makes, in miniature. It renders `[liveSummary]="false"` beside `DataTable`'s own `[summary]`, because both would otherwise be live regions carrying the same sentence, and a screen reader would announce the row count twice on every page change.

A truncated breakdown (`groupsTruncated`, past `Reports:MaxGroups`) renders its own notice above the table rather than folding into the count summary, because it is a fact about the *server's* answer, not about the page on screen — a reader comparing the visible rows against the KPI tiles above needs the reason stated plainly, not implied by a mismatched arithmetic.

### 12.5 Formatting: tokens, cost, and null versus zero

`usage-format.ts` carries every number the dashboard shows, in one fixed `en-US` locale — the app ships no `LOCALE_ID` and no i18n, so one consistent grouping beats a per-browser one. Two rules matter beyond the obvious thousands-separator and `48.2M`-style abbreviation:

- **Cost renders as an em dash, never `$0`, when `estimatedCost` is `null`.** This is the client half of the same distinction the server's `EstimatedCost` makes ([Usage and Favourites](../conversations/usage-and-favorites.md#7-reporting)): a period nobody could price and a period that cost nothing are different facts, and rendering the first as `$0` would claim the calls were free.
- **Sub-$10 figures keep their cents.** The board only ever draws whole dollars, but a quiet week on a cheap model rounds to `$0` without them, and a cost report that reads zero when money was in fact spent is worse than one that is two characters wider.

### 12.6 Empty, loading and error states

A range with no traffic (`totals.turns === 0`) renders a **named** empty state — "No usage between …" — rather than the dashboard with zeroes in it: four zero tiles, a flat ridgeline and an arc-less donut for the length of a quiet month are visually indistinguishable from the same shapes drawn from a genuine outage, which is exactly what US-1302's own fifth acceptance criterion asks the empty state to prevent. (This is a different "criterion 5" from US-1403's, in §12.4 above — the two stories happen to share a number for an unrelated criterion each.) `isEmpty` is computed from the **response**, not from request status — gating it on `isFulfilled` would go false the instant a re-range put the store back into `pending`, and the template would fall through to the dashboard branch for the length of that request, painting the exact chart-of-zeroes the empty state exists to prevent.

While a request is in flight over an existing report, the dashboard stays on screen, dimmed (`aria-busy`, `.admin-reports__dashboard--busy`) rather than replaced by a skeleton — the skeleton is reserved for the *first* load (`isFirstLoad`, request pending with no report yet). The controls stay operable throughout: disabling them under the reader's own gesture used to blur the control, which drops a keyboard user to the top of the document, and on the closed `<select>` — which commits on the first arrow key — makes the rest of the list unreachable. `switchMap` in `ReportsStore` already discards whatever a superseded request would have returned.

A failed request replaces the whole dashboard rather than sitting beside it, through `ErrorPanel` carrying the `traceId` and a retry — because a chart left on screen next to a failure notice is a chart of the *last* range the reader looked at, now labelled with the range that just failed.

## 13. Route, bundle and the shared kit
...
That last row is the shape to aim for and the reason §3.4's "no store" decision was not only about FR-49: a tab that adds nothing to `core/` adds nothing to the initial graph, whatever it renders. **US-1302 keeps the shape**: `ReportsStore` is provided on `AdminReports` (§12), not on anything reachable from the initial graph, so every reports file — the store, the five presentational components, `usage-format.ts` — rides the lazy `admin-routes` chunk exactly as the panel it replaced did. By 2026-08-21 the initial bundle stands at **672.06 kB raw / 169.20 kB transfer** against the unchanged 675/720 kB budget and **64.53 kB** `styles` against 66/80 kB — up **0.07 kB** from **671.99 kB**, none of it attributable to this story; the total also carries every P8/P9 story that landed between US-1209 and this one, none of which this table itemises. The lazy `admin-routes` chunk itself grew **95.9 → 127.13 kB**, and the admin area remains absent from the initial graph.

**The note left above for US-1302 is now answered.** No charting library landed — §12.3 is inline SVG and CSS throughout — so the predicted cost of a static `component:` import shared by all four tabs never materialised, and `admin.routes.ts` keeps every tab's static import unchanged. Switching the reports route to `loadComponent` remains available the day a future story does add a heavy per-tab dependency.

The two event-handler deltas from US-1207/US-1208 are worth reading together, because they are the shape of every future cross-boundary event and they are **not the same size**. Both stores are root-scoped and genuinely initial — the chat composer reads each — so composing `withEventHandlers` onto one puts that handler, and the `adminEvents` symbol it names, on the initial graph. US-1207 paid 0.46 kB because it brought `withEventHandlers` and the event group with it; US-1208 paid 0.15 kB because both were already there and only the handler itself was new. `ReportsStore` composes neither — §12's opening note is why — which is the other half of why it costs the initial graph nothing at all.

### 13.2 The kit had been waiting for this epic
...
| Honesty                 | `admin-users` (3)         | **No sort control, no selection column, and the interim "Filters loaded rows only." caption never rendered**; the order stated with its explanation reachable; **the count announced once**, by the table and not the pager                                                                                                                                                                                                                                                                                                                             |
...
| The form dialog     | `mcp-server-form-dialog` (18)   | **The five fields the API accepts, and no linked-permission control**; the note saying why a name can collide with a permission; **three auth types, not frame `5h`'s** (an equal count by coincidence — different values); **Scope shown only for Entra ID, and cleared when the arm stops accepting one**; an edit of a `None` server carrying a scope still submittable; an unsupported auth type explained **without waiting to be touched**, and recovering; the auth type sent as the number the wire carries; the server's message at its field, cleared on the next edit; a second open empty |
| The form's rules    | `mcp-server-form` (17)          | The three auth types and an unknown one labelled by its raw value; `IsInEnum`'s zero refused; `urlError` over both schemes, relative paths and trimming; **`scopeError` in both directions**; a create seeded with `None`; **`toMcpServerBody` sending a null scope whatever the hidden field holds**, trimming, and refusing a body it cannot build; `describeRejectedFields` saying nothing rather than naming a field the reader cannot find                                                                                                        |
...
| Area              | Spec                  | Notable cases                                                                                                                                                                                                                                                                                                        |
| ----------------- | --------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| The reports tab   | `admin-reports` (5; superseded by US-1302, below) | Tested the interim `UnavailablePanel` placeholder — the tab named at level one, the missing-capability copy in frame `5i`'s words, nothing to press, no request issued at all. None of these cases exist any more; `admin-reports` now tests the live dashboard |
...
And US-1206's, on the directory's own toolbar (15 net specs: 11 on `admin-users`/`admin-users-route`, 4 on `admin-users-store`; restated here rather than as a new suite total, which had already moved past 1664 from unrelated work landing after US-1209):

| Area                | Spec                            | Notable cases                                                                                                                                                                                                                                                                                                                                                       |
| ------------------- | -------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| The route helper    | `admin-users-route` (1 new)     | `toPermissionFilter` shape-only, an empty or all-whitespace value reading as no filter, and no opinion on whether an id names anything real                                                                                                                                                                                                                        |
| The screen          | `admin-users` (10 new)          | The catalogue's own order, "Permission: any" first; **the chosen permission in the URL, the page dropped, and the request narrowed**; **a deep link's permission on one request, with no unfiltered flash**; **an id the catalogue does not list read as "any" and repaired out of the URL**; the term and the permission both named when both produced the empty state, and each alone when only one did; **an unnameable filter kept in force, rendered as its own option, and still clearable**; turning the filter on pushing, swapping one permission for another replacing                                              |
| The store           | `admin-users-store` (4 new)     | `permissionId` sent only while the filter is on; **the filter surviving a retry**; **the request withheld while the query is unresolved, then sent once it resolves**; the permission blamed rather than an empty search box when nothing holds it                                                                                                               |

And US-1302's, on the reports dashboard (71 net specs: 16 more on `admin-reports`, replacing the interim panel's 5, plus six new files — `admin-reports-store` (12), `kpi-tile` (4), `outcome-donut` (7), `range-segmented` (5), `usage-area-chart` (10), `usage-bar-list` (6) — and `usage-format` (11); restated here rather than as a new suite total, which had already moved well past 1664 from unrelated work landing after US-1209):

| Area                  | Spec                     | Notable cases                                                                                                                                                                                                                                                                                                                                                     |
| ---------------------- | ------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| The screen              | `admin-reports` (21)      | **`?range=` and `?group=` bound as one computation, issuing one request for a deep link**; the range control always pushing and the grouping following the directory's push-or-replace rule; the date chip reading the response's window, never the control's; **the empty state naming the range**; the busy dashboard staying operable and dimmed rather than replaced by a skeleton mid-request; a failed request replacing the whole dashboard, never sitting beside it; the table header renamed after the response's own `groupBy`; the six numeric columns turning proportional below the desktop edge (US-1403) |
| The store               | `admin-reports-store` (12) | `switchMap` discarding a superseded range or grouping change; `bindQuery` firing once for both parameters together; the page reset to 1 on every new report; `setPage` clamped to the response's own page count; `retry()` replaying the last query rather than the default one; sign-out cancelling and clearing |
| The KPI tile            | `kpi-tile` (4)             | The arrow suppressed on an unchanged figure rather than lying about direction; a `null` delta rendering "No … to compare with" instead of a percentage; direction stated in text as well as colour (SC 1.4.1) |
| The outcome donut       | `outcome-donut` (7)        | The ring `aria-hidden`, with the legend carrying the name, count and share of every arc; an outcome with no calls simply absent, not a zero-length arc; the combined cancelled-and-failed share and its token total at the centre |
| The range control       | `range-segmented` (5)      | The active value marked without disabling the others; a press emitting exactly the range pressed |
| The area chart          | `usage-area-chart` (10)    | The `aria-hidden` path paired with a computed summary sentence — span, total, peak day; a one-point series drawing a visible zero-length segment rather than nothing (SVG 2 §13); a fresh gradient id per instance, so a second chart cannot repoint the first chart's fill |
...
And a bug-fix pass closing six defects in `DataTable`'s card anatomy, found after US-1403 (§3.3.1) — restated here rather than as a new suite total, which has moved well past 1664 from unrelated work landing since:

| Area                    | Spec                    | Notable cases                                                                                                                                                                                                     |
| ------------------------ | ------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| The shared kit           | `data` (4 new)           | The `meta` slot rendering a `<dt>`/`<dd>` pair per column, keyed by its header; the `trailing` slot rendering beside the title rather than in the card's foot; a `text`-only column no longer blank on a card; a `hideOnTablet` column absent from the table between 768 and 1023px and present on the card |
| The catalog rows         | `admin-models` (2 new)   | The deployment, context and output figures each labelled on a card; deployment, output and tools absent from the table at tablet width                                                                        |
| The registry rows        | `admin-mcps` (2 new)     | The URL, auth and scope each labelled on a card; description, auth and scope absent from the table at tablet width                                                                                             |
| The conversations list   | `conversations` (1 new)  | The favourite star and the kebab both rendered in the card's `trailing` slot, not its full-width foot                                                                                                          |
...
| **Header key and OAuth2 auth types**         | —                                                                                           | `McpAuthTypes` has three members — `None`, `EntraIdOnBehalfOf` and `UserApiKey` — and neither of the board's is among them. `UserApiKey` is not a late arrival of the board's own "Header key": that refusal was one shared secret an administrator typed into a plaintext, administrator-readable column, where `UserApiKey`'s credential belongs to the user, stored encrypted per user, and never appears on this dialog at all (§11.3). Offering either of the board's two as a further member would still be a select entry the wire refuses. Not the **Headers** field below it in the same dialog either — that carries non-secret per-server configuration, unaffected by which of the three auth types the server uses (§11.3.2) |
...
The last two catalog absences and four of the registry's are recorded in the [build order](../prd/enterprise-ui-rebuild-build-order.md)'s interim-behaviours table, alongside the directory's Last active column.
...
| **A deep link to `?size=100` requests 100 rows and displays "25 / page"**                   | The select's selection was bound as `[value]` on the `<select>` instead of `[selected]` on the options; the empty option list resets it to the first entry (§13.2)    |
...
| The whole admin area lands in the initial bundle                                            | Something eagerly reachable imported one of the three root action stores, or a file beside one. `providedIn: 'root'` is a DI scope, not a chunk assignment (§13.1)    |
...
| One of the table tabs starts stretching or shrinking oddly                                  | `.admin__content` is a flex column (originally for frame `5i`'s centred panel) and a tab's host set something other than the default `flex` a blockified item gets (§3.4)           |
| A deep link to `?group=user` shows "Model" in the closed Group-by select                    | `[value]` was bound on the `<select>` instead of `[selected]` on each `<option>` — that binding runs before `@for` inserts the options and resets to the first one (§12.1) |
| The date chip names a range the dashboard is not actually showing                           | It was derived from `?range=` instead of the response's own `from`/`to` — the control flips the instant it is clicked, before the next report lands (§12.2)          |
| A card's row buttons stay under the 44px touch target                                       | `card-row.scss`'s shared rule reaches only what a consumer projects into a slot, never past a wrapper — a screen using the `trailing` slot or its own `actions` foot must size its own buttons (§3.3.1)                                                              |
| An empty card slot still shows a blank gap where a section should have collapsed             | The empty-slot rule needs `:has(> :only-child:empty)`, not bare `:empty` — `DataTable` always projects a wrapper into every slot, even an empty one (§3.3.1)          |
...
| **A card under this tab stacks deployment, context and output with nothing telling them apart** | The `meta` slot's `<dl>` labelling is `DataTable`'s own template, keyed by `column.header` — check it did not regress there before assuming this screen (§3.3.1) |
| The table renders all eight columns at tablet width and runs wider than the pane | A column lost `hideOnTablet` — deployment, output and tools carry it here — or `visibleColumns` came off the table branch in favour of `columns()` (§3.3.1, §10.3) |
| A deployment or model name is clipped instead of wrapped on a card       | Its `bp.mobile` wrap rule came off, or `.admin-models__name` lost the `min-width: 0` it also needs at narrow desktop widths, independently of the card (§3.3.1, §10.3) |
...
| **A deactivated server's row never dims**                                                      | The template applies `.admin-mcps__server--retired` — check the stylesheet still defines that class and not `.admin-mcps__name--retired` (§3.3.1)                                            |
| **A card under this tab stacks URL, auth and scope with nothing telling them apart**            | Same `meta`-slot labelling as the models tab — it is `DataTable`'s own template, not this screen's (§3.3.1)                                                                                    |
| The table renders all eight columns at tablet width                                             | A column lost `hideOnTablet` — description, auth and scope carry it here (§3.3.1, §11.2)                                                                                                       |
| A URL or scope is clipped instead of wrapped on a card                                          | `.admin-mcps__muted`'s `bp.mobile` wrap rule came off (§3.3.1)                                                                                                                                  |
| A long permission name overflows its card                                                       | `PermissionBadge` lost `max-width: 100%`, or its name span lost the ellipsis-then-wrap pair its own `bp.mobile` block draws (§3.3.1)                                                            |
...
| The reports dashboard           | [`features/admin/reports/`](../../enterprise-gpt-ui/src/app/features/admin/reports/) — `admin-reports.ts`, `.html`, `admin-reports-store.ts`, `admin-reports-route.ts`, `usage-format.ts`, and the five chart/tile components (§12)      |
...
| The wire shapes                | [`domain/api/user.ts`](../../enterprise-gpt-ui/src/app/domain/api/user.ts), [`domain/api/model.ts`](../../enterprise-gpt-ui/src/app/domain/api/model.ts), [`domain/api/provider.ts`](../../enterprise-gpt-ui/src/app/domain/api/provider.ts), [`domain/api/mcp.ts`](../../enterprise-gpt-ui/src/app/domain/api/mcp.ts), [`domain/api/usage-report.ts`](../../enterprise-gpt-ui/src/app/domain/api/usage-report.ts)                                                                              |
