...
| DELETE | `/api/models/{id:guid}` | **admin** | 204 — soft delete (sets `DateDeactivated`) | 403 not admin, 404 unknown/already deactivated, 409 target is the configured document summarizer |
...
`dateDeactivated` is always `null` on user-facing endpoints (they filter to active rows); the admin listing uses it to distinguish toggled-off models. `isUserSelectable` is always `true` on `GET /api/models`, which filters on it (§2.4) — it is only ever informative on `GET /api/models/all` and `GET /api/models/{id}`.

Request DTOs — [`ModelActions.cs`](../../enterprise-gpt-api/Enterprise.Gpt.Dto/Actions/Model/ModelActions.cs). `CreateModelActionDto` and `UpdateModelActionDto` share the same shape (`providerId`, `name`, `deploymentName`, `description`, `contextWindowSize`, `maxOutputTokens`, `isToolEnabled`, `isReasoningEnabled`, `isUserSelectable`, `isDefault`). The update DTO carries **no id** — it comes from the route.

`isReasoningEnabled` is additive and defaults to `false`, so a client that omits it keeps the old behaviour on create — but remember that `PUT` replaces the whole model, so an update that omits it turns reasoning **off** (§2.3). `isUserSelectable` defaults the **other** way, to `true` (§2.4): both are request-DTO initializers rather than server-side patch semantics, and the two flags pick opposite defaults deliberately, because the CLR default for either would silently produce the wrong effect — `false` for `isReasoningEnabled` costs nothing extra, while `false` for `isUserSelectable` would hide a model nobody asked to hide.
...
### 2.4 `isUserSelectable`

A picker-**visibility** switch, independent of retiring a model. Column: `Core.Ref.Model.IsUserSelectable`, `bit NOT NULL DEFAULT 1`, added by migration `20260825031549_AddModelIsUserSelectable` and defaulted to `true` for every pre-existing row so the migration that introduces the column cannot silently hide the catalog.

- **It governs `GET /api/models` — the chat picker's source — and nothing else.** `GET /api/models/all` (the admin listing) and `GET /api/models/{id}` both ignore it, so a hidden row stays findable and editable from the admin screen. This is the same two-routes-two-meanings split `dateDeactivated` already has between the two listings, and the two flags are orthogonal: a row can be active-and-hidden, active-and-visible, or retired, but never "hidden because retired" — retiring is still `dateDeactivated`'s job.
- **It is visibility, not authorization.** A chat turn that names a hidden model by `modelId` directly — bypassing the picker — is served normally: the turn's own model lookup filters on `dateDeactivated` alone and does not consult this flag. Hiding a row keeps it out of the list a user browses; it does not revoke it.
- **`IsDefault && !IsUserSelectable` is rejected** by both `CreateModelActionDtoValidator` and `UpdateModelActionDtoValidator` — 400, keyed to `IsDefault`. A hidden default would resolve to nothing on `GET /api/models` and leave every composer with no model to start a turn on; the request states two intentions and only the caller knows which to keep, so the combination is refused rather than silently corrected.
- **Toggled from the admin models screen** (`features/admin/models/model-form-dialog.ts`) as a "Visible in model picker" switch beside the existing tool and reasoning toggles, defaulting on for a new model. The admin models table (`admin-models.html`) draws a third status dot, **Hidden**, for a row that is active but not user-selectable — distinct from **Active** and from **Retired** (`dateDeactivated` set), so the column still says exactly one thing per row. The row action menu withholds "Set as default" from a hidden row, mirroring the server-side rule above.
- **First real use: the pinned document summarizer.** The seeded row `c36e22ed-262a-47a1-b2ba-06a38355ae0f` ships with `IsUserSelectable = false` (migration `20260825031604_CorrectSummarizerModelSeed`) so it never appears as something a user can pick to chat with, while still being resolvable by id for the summarization work that is pinned to it. See [Document Summarizer Configuration](../summarization/model-configuration.md).
...
| `isDefault` | rejected (`false` required) when `isUserSelectable` is `false` |
...
Two more checks apply only to the row named by `Summarization:ModelId` — the pinned document summarizer — and only in the service, because the validator has no way to know which row that is: a `PUT` that changes its `providerId` to one with no registered chat client is rejected the same way, 400 keyed to `providerId`, and a `DELETE` targeting it is refused outright (§7). See [Document Summarizer Configuration](../summarization/model-configuration.md).
...
**One row refuses deletion outright: the configured document summarizer.** Before the `ExecuteUpdateAsync` above runs, `DeactivateModelAsync` checks the target id against `Summarization:ModelId` and throws `SummarizerProtectedException` — 409, no domain problem type, `detail` naming the setting to change — if they match. Retiring that row would make the application fail to start on its next restart (the startup check treats a deactivated row as absent, §5 above and [Document Summarizer Configuration](../summarization/model-configuration.md)), and the migration that seeded the row is already applied, so a redeploy does not put it back. Recovery from a bricked instance would mean hand-editing the database; the guard exists so that path is never needed. Which catalog row is the summarizer is an operator's configuration decision, not one the admin catalog screen administers — an administrator can still rename, reprice, or hide/show that row like any other, just not delete it or move it to an unservable provider while it is pinned.
...
| `isUserSelectable` column and migrations | [`Enterprise.Gpt.Entity/Model.cs`](../../enterprise-gpt-api/Enterprise.Gpt.Entity/Model.cs), [`Migrations/20260825031549_AddModelIsUserSelectable.cs`](../../enterprise-gpt-api/Enterprise.Gpt.Repository/Migrations/20260825031549_AddModelIsUserSelectable.cs), [`Migrations/20260825031604_CorrectSummarizerModelSeed.cs`](../../enterprise-gpt-api/Enterprise.Gpt.Repository/Migrations/20260825031604_CorrectSummarizerModelSeed.cs) |
| Frontend consumers | [`enterprise-gpt-ui/src/app/core/catalog/model-catalog-store.ts`](../../enterprise-gpt-ui/src/app/core/catalog/model-catalog-store.ts) (`GET /api/models`, for the chat picker), [`model-actions-store.ts`](../../enterprise-gpt-ui/src/app/core/catalog/model-actions-store.ts) and [`features/admin/models/`](../../enterprise-gpt-ui/src/app/features/admin/models/) (the admin tab) — see [Administration §10](../ui/administration.md#10-the-model-catalog-us-1207) |
| Document summarizer configuration and guards | [Document Summarizer Configuration](../summarization/model-configuration.md) |

## 9. Known gaps and extension points

- **Reactivation endpoint:** `PUT` targets active rows only, so a deactivated model currently has no "toggle back on" path; add e.g. `POST /api/models/{id}/activate` (admin) that clears `DateDeactivated`. This is now visible in the product: the admin models tab renders retired models (they are what this route's soft delete produces) and deliberately offers **no** action on them at all, rather than a Restore that could not work — recorded as an interim behaviour in the UI build order ([Administration §10.3](../ui/administration.md#103-the-table-and-why-a-retired-model-is-still-in-it)).
...
- ~~**Seed data:**~~ **Corrected by migration `20260825031604_CorrectSummarizerModelSeed`** (2026-08-25). The seed used to leave `ContextWindowSize`/`MaxOutputTokens` at `0`, which the validators reject, so the seeded model could not round-trip through `PUT` until real values were seeded. It now carries `DeploymentName = "rr-gpt5.6-luna"`, `ContextWindowSize = 1000000`, `MaxOutputTokens = 16384`, and `IsUserSelectable = false` — it round-trips through `PUT` like any other row, and it is the pinned document summarizer these values were corrected *for*: see [Document Summarizer Configuration](../summarization/model-configuration.md). One consequence worth stating: this is now the **only** model this application seeds, and it is hidden from the picker — a database built from empty has an empty `GET /api/models` until an administrator adds a chat model by hand.
...
- ~~**Admin UI:**~~ **Built by US-1207** (2026-08-16). `enterprise-gpt-ui` now consumes all five routes: `GET /api/models` for the chat model picker and `GET /api/models/all`, `POST`, `PUT` and `DELETE` for the administration tab — see [Administration §10](../ui/administration.md#10-the-model-catalog-us-1207). Reuse `PermissionEndpointFilter.Require(PermissionIds.Administrator)` for any future admin-only endpoints (provider CRUD, user permission management).
- **No provider listing, and the client mirrors the seed instead.** Nothing exposes provider names over HTTP to any caller, administrator included, so the admin form's Provider select is built from a hand-maintained mirror of `Providers.cs` in [`domain/api/provider.ts`](../../enterprise-gpt-ui/src/app/domain/api/provider.ts). It is safe only because the seeded ids are documented as matching the enum verbatim — but it means a provider row added server-side after a client build renders as a bare GUID and cannot be chosen. A `GET /api/providers` returning the active rows would retire the mirror. (Until then, note that offering a deactivated provider fails honestly: `EnsureProviderExistsAsync` answers 404 and the client renders it at the field.)
- **The whole-token rule lives only in the client.** `ContextWindowSize` and `MaxOutputTokens` are `decimal(18,2)` and their validator carries only `GreaterThan(0)`, so `400000.5` is accepted and stored; the admin form refuses it, and the table renders one decimal so a value seeded out of band is visible rather than rounded away. A `Must(v => v % 1 == 0)` rule — or an integer column — would close it server-side.
