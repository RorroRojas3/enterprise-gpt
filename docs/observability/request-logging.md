...
- **All three instruments are built in the constructor**, not on first use. Three instruments on a singleton cost nothing, where a lazily initialised field raced by two threads publishes the same instrument twice and the OpenTelemetry SDK reports a duplicate-instrument conflict. `ChatMetrics` is deliberately **not** `IDisposable` either: `IMeterFactory` owns the lifetime of every meter it creates, so disposing one from here would have no effect.
