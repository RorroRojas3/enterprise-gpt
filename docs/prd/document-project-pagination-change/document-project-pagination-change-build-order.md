...
Status here mirrors the `Status` field on each PRD story. Update both, or update the PRD and re-derive this. All eight stories landed on 2026-08-30, in wave order.
...
| --- | --- | --- | --- | --- | --- | --- |
| 1 | 1 | US-101 `[enabler]` Add server-side `name` filtering to `GET api/documents` | — | P0 | US-301 | **Done** |
| 1 | 2 | US-301 `[enabler]` Rebuild `ProjectListStore` on `withOffsetPagination` and Load more | — | P0 | US-101 | **Done** |
...
| Wave | Order | Story | Depends on | Pri | Parallel with | Status |
| --- | --- | --- | --- | --- | --- | --- |
| 2 | 3 | US-102 Grouped counts state how many of a conversation's documents match the active filters | US-101 | P1 | US-201, US-302 | **Done** |
| 2 | 4 | US-201 `[enabler]` Rebuild `DocumentLibraryStore` on `withOffsetPagination` and Load more | US-101 | P0 | US-102, US-302 | **Done** |
| 2 | 5 | US-302 Load more control matches the Conversations library's keyboard operability and focus restoration, adapted to a card grid | US-301 | P1 | US-102, US-201 | **Done** |
...
| Wave | Order | Story | Depends on | Pri | Parallel with | Status |
| --- | --- | --- | --- | --- | --- | --- |
| 3 | 6 | US-202 Grouped view groups only what is loaded, worded honestly | US-201, US-102 | P1 | US-203 | **Done** |
| 3 | 7 | US-203 Load more control matches the Conversations library's keyboard operability and focus restoration | US-201 | P1 | US-202 | **Done** |

✅ **This is the last wave, and it is closed.** All 8 stories in the PRD are done, and both screens carry the same paging model Conversations already ships.
