...
**Progress: 60 / 94 done.** Current position: P5, `US-307` — P4 is complete, EP-6 with it, EP-8 is complete, and EP-9 is complete except US-307 (row 61) and US-908 (row 62). What is left of EP-7 is US-307's project chip and kebab and the US-706 enabler that retires US-705's stated order.
...
**EP-9 completed 2026-08-14, in the numbered order — the one epic in P5 that had to be.** US-903 needs a grid to be invoked from; US-904 builds the detail screen; and US-905 and US-906 both add to that screen's tab strip and its composer slot, which frame `4e` draws on one page. Two worktrees would have collided on the same three files for no gain.
...
| `composer--streaming` dims `.composer__aux` — the microphone and the attach control carry it; the conversation download does not exist yet                       | US-407 (P3)                                 | US-1502 adopts the class when it lands (US-413 did, 2026-08-13; US-801 did, 2026-08-14)                                              |
...
| ~~The library offers no row selection or bulk bar~~ (the **per-row kebab** was always US-307's, and is still absent)                                              | US-701 (P5)                                 | ✅ selection and the bulk bar retired by US-704, 2026-08-14; the kebab, with the project chip, by US-307 (P5)                        |
...
| The project dialogs mount on `ProjectsLayout` rather than the shell, because both invokers are still inside the feature                                           | US-903 (P5)                                 | US-307 (P5), which adds the first one outside it                                                                                     |
| The project detail tab strip has no Conversations tab                                                                                                            | US-904 (P5)                                 | US-908 (P5)                                                                                                                          |
| The project composer has no project picker — the project is fixed by the route                                                                                   | US-906 (P5)                                 | US-307 (P5)                                                                                                                          |
| `SessionBootstrap` still does not request projects (US-202's fourth criterion), because no root project list has a consumer yet                                    | pre-EP-9                                    | US-307 (P5), the first consumer of one                                                                                               |
