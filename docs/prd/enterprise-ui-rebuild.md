...
- **Status**: ✅ Done (2026-08-13). Criterion 1 was already structural — `authErrorDecision` refreshes only the `http` arm at 401, so this 403 cannot enter the refresh path however it is handled — and now carries a chat-level regression test beside the policy-level one. Frame `1h`'s current variant ships: the plain `--surface` card, a `--warn` shield, the title in body colour, and **no action**, because the `scope` a consent request needs is null on every 403 this build can receive. The future variant is deliberately not built rather than built-and-dead: it arrives with US-411, which puts the scope on the wire. Retry is withheld by `canRetry` rather than by the card, which review found had **no test anywhere** — that table is now pinned, in both directions, since it is the single fact keeping this card from growing a dead button and US-408's from losing its live one. `userMessage` changed for this arm too: it said the server "needs _your_ authorization", which is false here and was the only thing a screen-reader user heard, the card not being a live region.
- **Acceptance criteria**:
...
- **Acceptance criteria**:
  - Given US-1301 has not landed, when the reports tab is opened, then `UnavailablePanel` renders frame `5i` — the ridgeline motif, "Usage reports aren't available yet", and a line stating the reporting API is not enabled for this deployment — and `ReportsStore` holds zero records: no sample chart, no placeholder totals.
...
- **Does adding columns for B3, B4, and B7 require the first EF Core migration?** `Repository/Migrations/` is empty and `Database.Migrate()` runs at startup; the repository's standards say to flag before adding the first migration because it changes startup behavior — _backend engineer, before US-909 or US-1101 starts_.
...
- **Which server-side stack renders `.docx` and `.pdf` for US-1501?** Markdown is trivial; the other two need a document library, a headless renderer, or a hosted service, and the choice drives the L estimate, the deployment footprint, and whether the route can stream its response or must return a signed URL. The typed "renderer not configured" problem in US-1501 exists precisely because this may differ per deployment — _backend engineer, before US-1501 is estimated_.
- **Does US-1301's contract cover everything frames `5j` and `5k` render?** The dashboard needs grouping by model, provider, user, and day, the completed/cancelled/failed split, per-model token and estimated-cost figures, and prior-period deltas for the four KPI tiles. US-1301 now asserts all of that, but it must be verified against the real `ConversationUsage` shape before P8, or the dashboard cannot be built from the enabler that is supposed to release it — _backend engineer, before US-1301 starts_.
- **Does `--warn` need to change in the design bundle?** In the light theme `#A96A10` measures 4.41:1 on `#FFFFFF`, 4.21:1 on `--bs-body-bg`, and 3.99:1 on its own `--warn-bg` — all below the 4.5:1 AA minimum for text below 18.66px — and it is used as `color` in the badge and chip stylesheets. The dark theme passes. The value is transcribed verbatim from `docs/design/project/theme.css` and `scripts/check-tokens.mjs` enforces that parity, so the fix belongs upstream in the design bundle rather than as a local override in the app — _whoever owns `docs/design/`, before EP-14's axe gate_.
- **Can the UI CI checks be marked required while the workflow keeps its `paths` filters?** `.github/workflows/ui-ci.yml` filters on `enterprise-gpt-ui/**` and three other paths. GitHub's documented behaviour is that a workflow skipped by a path filter never reports at all, so its checks stay Pending and block merging — making these checks required today would render every API-only pull request unmergeable. The resolution is either a skip-shim job or dropping the filters, and it should be settled before branch protection is turned on — _whoever configures branch protection_.
