...
- ~~**What is the initial-bundle budget in bytes?**~~ **Answered at the end of US-101 (2026-08-09); re-stated at the end of EP-1 (2026-08-10).** The first production build of the scaffold measured **220.34 kB raw / 59.64 kB estimated transfer** against a 240 kB warn / 300 kB error budget, with the router, `HttpClient`, and the US-102/US-103 foundation in but no feature code. With EP-1 complete the same build measures **349.83 kB raw / 88.13 kB estimated transfer**, and `angular.json` now carries **360 kB warn / 400 kB error** on initial raw plus a `styles` bundle budget at **65 kB warn / 80 kB error**, against a global stylesheet measuring 60.32 kB. Two things drove the jump: Bootstrap's SCSS subset, and `app.routes.ts` gaining a real route, so the router is no longer tree-shaken away. MSAL lands in EP-2 at roughly 150 kB and is unavoidably initial, so **US-201 needs its own re-baseline past the 400 kB error ceiling** rather than an exemption. Re-baseline and re-state this figure whenever a story adds one.
...
- **Does the initial bundle still fit under budget after `ngx-markdown`?** Still open, and now measurable: US-101 installed `ngx-markdown` 21.3.0, marked, and Prism, but nothing imports them, so the 349.83 kB baseline above is still a pre-markdown figure and the cost is unpaid. `ngx-markdown`, marked, and Prism are a substantial initial-chunk addition against that baseline and a 400 kB cap that MSAL already spends against first. Either the baseline is re-measured and re-stated per US-601, or the transcript renderer moves behind the lazy chat route — the decision changes the shape of the chat route, so it is worth taking before US-601 rather than after — *frontend engineer, at the start of US-601*.
