...
- **The rule that splits the two surfaces is the extension, and it is stated once, here.** An `attachments[]` entry whose extension is an image renders through this PRD's preview grid; an entry with any other extension renders through the generated-**document** chip `docs/prd/file-agent/file-agent.md` ships for the `.docx`, `.xlsx`, `.pptx` and `.pdf` artifacts the File Agent produces. Both surfaces read the same array on the same message, so the discriminating rule has to live in one place — inferred separately in two documents, they can both claim an attachment or neither can, and the failure mode is a file that renders twice or not at all.
...
- **An inline preview must not put a SAS in the DOM.** The standing posture is that a signed link is never prefetched, persisted, or written into a URL (`docs/documents/download-workflow.md` §5.4), and it is already the shipped behaviour: `core/documents/document-download-store.ts` issues **no** request until a chip is clicked, so a transcript full of files costs zero signed links until one is asked for. An `<img src="{sas}">` on every image in a transcript breaks that rule outright — it would mint a credential per image on render, for images the user may never look at. The proposed resolution is **open question 1**: a new authenticated byte-streaming route serves previews while the SAS route stays for explicit downloads only.
...
- **Acceptance criteria**:
  ...
  - Given an `attachments[]` entry whose extension is **not** an image, when the message renders, then this component ignores it entirely and renders nothing for it — that entry belongs to the generated-document chip `docs/prd/file-agent/file-agent.md` ships, and the extension is the only thing that decides which surface owns an attachment.
...
**This PRD is a hard predecessor of `docs/prd/file-agent/file-agent.md`.** That PRD's "The File Agent" epic cannot start until EP-1 and EP-2 here close: it needs the document-type discriminator, the `generated` container and the non-ingesting write path, its own generated-document chip is built on **US-104** (the download route and its content-type map) and **US-105** (the `attachments[]` array on the assistant message), and its "embed a generated image in a document" story depends on **US-202** and **US-203** — one implementation of the image function, reached from two callers. The sandbox has no outbound network access, so the File Agent cannot call an image API for itself; the API places bytes into its container from what this tool returned.
