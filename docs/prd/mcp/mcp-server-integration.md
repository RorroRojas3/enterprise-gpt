...
- ~~Renaming `Scope` to `Scopes` on the *administrative* `McpServerDto` and on `CreateMcpServerActionDto`/`UpdateMcpServerActionDto` (US-101) is a deliberate breaking change to that contract, not an additive one. It is taken knowingly because the admin surface has no client yet — `docs/prd/enterprise-ui-rebuild.md`'s US-1208 (admin MCP CRUD) is unstarted — and the environment is pre-production.~~ **Expired 2026-08-17: US-1208 shipped, and the administrative contract now has a client.** The rename stays a deliberate breaking change, but it is no longer breaking nothing: `enterprise-gpt-ui`'s `domain/api/mcp.ts` (`McpServerDto.scope`, `McpServerWriteBody.scope`), `core/catalog/mcp-server-form.ts` (`scopeError`, `toMcpServerFormValue`, `toMcpServerBody`) and the form dialog's conditional Scope field all read and write the singular field, and the frontend additionally binds **one** scope where `Scopes` is a list. **US-101 therefore has to carry the frontend change with it** — a single-field rename plus a single-to-multi widening on that form — rather than landing alone; the environment being pre-production is what keeps this a coordinated change rather than a versioned contract. It is recorded as an interim behaviour ("Single scope only") in `docs/prd/enterprise-ui-rebuild-build-order.md`, retired by this story. The *user-facing* `McpDto` change in US-203 remains strictly additive, because that listing does have a live consumer.
