...
| --- | --- | --- | --- | --- | --- | --- |
| 1 | 1 | US-101 `[enabler]` Register the `.xlsx` extractor | — | P0 | US-102 | |
| 1 | 2 | US-102 `[enabler]` Register the `.csv` extractor | — | P1 | US-101 | |
| 1 | 3 | US-103 `[enabler]` A malformed or protected spreadsheet fails cleanly | US-101, US-102 | P1 | — | |
...
| Wave | Order | Story | Depends on | Pri | Parallel with | Status |
| --- | --- | --- | --- | --- | --- | --- |
| 4 | 17 | US-501 `[enabler]` Cap rows and columns accepted per sheet | US-201, US-202 | P0 | US-502, US-503, US-504 | |
...
⛔ **`US-501` should not wait for wave 4 to open in practice**, even though it is scheduled here for tracking: it depends only on wave 2's extractors and guards against a real cost/storage risk (§8's risks table). Teams with spare capacity during wave 2 should pull it forward rather than treating "wave 4" as a hard start gate.
