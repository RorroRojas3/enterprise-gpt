...
/**
 * Every document the signed-in user has uploaded, from `GET api/documents` — flat or
 * grouped by conversation (US-1002/US-1003, frame `4j`).
 *
 * It copies `ProjectListStore`'s drain minus the term — this route takes no filter, so
 * a drain is only ever superseded by a retry or a sign-out — with the same load-bearing
 * details: `expand` rather than a self-referential `concat`, `_querySuperseded$`
 * cancelling the whole chain, `truncated` written where the drain stops and never in a
 * `finalize`, and `takeUntil(signedOut$)` on the request because `withResetOnSignOut`
 ...
 * from the first page.
 *
 * The store holds **zero seeded records by construction** — a US-1002 criterion:
 * nothing is fetched until the screen calls {@link load}, and no placeholder row ever
 * ships in state, so a user who has uploaded nothing drains an empty first page and
 * lands on the empty state. Frame `4i`'s pre-enabler unavailable panel was deliberately
 * never built: US-1001 landed first, so no client ever shipped without the endpoint
 * behind this screen (product-owner decision 2026-08-19).
 *
...
export const DocumentLibraryStore = signalStore(
  withState<DocumentLibraryState>({ truncated: false }),
  ...
  withComputed(
    ({ entities, hasQuery, isEmptyResult, isFulfilled, isPending, results, totalCount }) => ({
      /**
      ...
      /**
       * Loaded, and this user has uploaded nothing at all — frame `4k`'s gate, distinct
       * from {@link hasNoMatches}: with zero documents "Nothing uploaded yet" is the
       * truth whatever term a deep link carries.
       */
      ...
      ceilingNotice: computed(
        () => `Showing the ${entities().length} most recent of ${totalCount()} documents.`,
      ),
    }),
  ),
  withMethods((store) => {
    function fetchPage(skip: number): Observable<PaginatedResponseDto<UserDocumentDto>> {
     ...
     * truncated one.
     */
    function nextPage(): Observable<PaginatedResponseDto<UserDocumentDto>> {
      if (!store.hasMore()) {
        return EMPTY;
      }
    ...
    return {
      /** Starts the drain. The screen calls it once, from its constructor. */
      load(): void {
        _load();
      },
