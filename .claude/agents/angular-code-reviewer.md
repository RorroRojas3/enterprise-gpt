...
- **Comments & docs** — a PRD story id, epic, requirement id, or design frame anywhere in source (`US-1402`, `FR-11`, "frame `2f`") is **High**. A multi-paragraph TSDoc header on a component, directive, or store, a comment that explains Angular or the language, or a `/** */` restating the symbol name is **Medium**. See `.claude/CLAUDE.md` §"Communication & comments".
