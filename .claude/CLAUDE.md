...
- **Bundle budget** — production build fails above 300 kB initial raw, warns above 240 kB; baseline is 219 kB. Re-baseline whenever a story adds a substantial initial-chunk dependency.
...
| `rxMethod` / `withEntities` / `protectedState` | No store exists yet — US-104 writes the composable features, US-302 sets the reference store pattern | Follow the skill from the first store; there is no legacy state service to work around |
