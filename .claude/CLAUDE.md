...
- **Bundle budget** — production build fails above 400 kB initial raw, warns above 360 kB, with a `styles` bundle budget at 65/80 kB; baseline is **349.83 kB raw / 88.13 kB transfer** as measured at the end of US-106 (Bootstrap's SCSS subset in, `ngx-markdown` still unimported, MSAL still unwired). MSAL is roughly 150 kB and unavoidably initial, so US-201 needs its own re-baseline past 400 kB. Re-baseline whenever a story adds a substantial initial-chunk dependency.
