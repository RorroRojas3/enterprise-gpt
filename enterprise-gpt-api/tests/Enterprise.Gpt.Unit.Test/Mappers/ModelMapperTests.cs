...
{
    private static Model CreateModel(DateTimeOffset? dateDeactivated = null, decimal? inputPrice = 2.5m, decimal? outputPrice = 10m)
    {
        return new Model
        {
            Id = Guid.NewGuid(),
    ...
    /// <summary>
    /// A price omitted from the request clears the stored one, which is what makes an unpriced
    /// model distinguishable from a free one under the full-representation PUT semantics.
    /// </summary>
    [Fact]
    public void FromUpdateModelActionDtoToModel_PricesOmitted_ClearsStoredPrices()
    {
        var model = CreateModel(inputPrice: 2.5m, outputPrice: 10m);
        var request = new UpdateModelActionDto
        {
    ...
    [Fact]
    public void MapToModelDto_UnpricedModel_MapsPricesAsNull()
    {
        var model = CreateModel(inputPrice: null, outputPrice: null);

        var dto = model.MapToModelDto();

        Assert.Null(dto.InputPricePerMillionTokens);
        Assert.Null(dto.OutputPricePerMillionTokens);
    }
}
